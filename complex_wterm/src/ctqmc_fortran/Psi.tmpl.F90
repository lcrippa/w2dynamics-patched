...
module MPSI
    ...
    type, public :: PSIS
        type(OPERATOR), allocatable :: Psis(:,:,:)
    end type
...
contains
    ...
    subroutine p_make_hamiltonian(muimp, u_matrix, states, psi, hloc, qnthr)
        VALUE_TYPE, intent(in) :: muimp(:, :, :, :), u_matrix(:, :, :, :)
        REAL_TYPE, intent(in) :: qnthr
        type(TStates), intent(inout) :: states
        type(PSIS), intent(inout) :: psi
        type(OPERATOR), intent(out) :: hloc
        type(TQNSpec) :: qnspec
    ...
    subroutine analyze_hamiltonian(dh, dstates, states2substates, threshold, log)
        ...
        REAL_TYPE, intent(in) :: threshold
        logical, intent(in) :: log
        REAL_TYPE :: maxignored
    ...
    subroutine add_oneparticle_hamiltonian(h, DPsis, muimp)
        ...
        type(PSIS), intent(in) :: dpsis
        VALUE_TYPE, intent(in) :: muimp(:,:,:,:)

        integer :: b1, b2, sp1, sp2, nbands
        VALUE_TYPE :: muimp_value
    ...
    subroutine add_u_hamiltonian(u_matrix, h, dpsis)
        type(OPERATOR), intent(inout) :: h
        type(PSIS), intent(in) :: dpsis
        VALUE_TYPE, intent(in) :: u_matrix(:,:,:,:)

        integer :: b1, b2, b3, b4, sp1, sp2, sp3, sp4, nbands
        VALUE_TYPE :: u_value
        type(OPERATOR) :: op12, op124, op1243
    ...
    subroutine p_transform_Psis(Eigen, transformed_Psis)
        ...
        type(PSIS), intent(inout) :: transformed_Psis
        integer :: iB, iS, CA
    ...
    function p_rho1_from_density_matrix(rho_mb, hloc, psis) result(rho1)
        type(OPERATOR), intent(in) :: rho_mb
        type(OPER_EIGEN), intent(in) :: hloc
        type(PSIS), intent(in) :: psis
        VALUE_TYPE, allocatable :: rho1(:, :)
        ...
        type(KET) :: ket1, ket2
        type(BRA) :: bra_rho
        type(OPERATOR) :: rho_dag
        VALUE_TYPE :: rho_entry
    ...
    function p_double_occ_from_density_matrix(rho_mb, hloc, psis) result(occ)
        ...
        type(OPER_EIGEN), intent(in) :: hloc
        type(PSIS), intent(in) :: psis
        VALUE_TYPE, allocatable :: occ(:, :, :, :)

        integer :: ib2, is2, ib1, is1, iSSt
        integer :: nsstates, nbands

        type(KET) :: ket, ket1, ket2, ketw
        type(BRA) :: bra_rho
        type(OPERATOR) :: rho_dag
        VALUE_TYPE :: rho_entry
