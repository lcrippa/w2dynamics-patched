...
module MSAMPLER
    ...
    type, public :: SAMPLE_BUFFER
        type(SAMPLE_POINT), allocatable :: buffer(:)
        integer :: n
    end type

    !> A sampling point, i.e., a tuple (i, x, f(x))
    type, public :: SAMPLE_POINT_2D
        integer(c_int64_t) :: i
        REAL_TYPE :: x(2)
        VALUE_TYPE :: fx
    end type SAMPLE_POINT_2D

    !> A buffer for the use of sampling points
    type, public :: SAMPLE_BUFFER_2D
        type(SAMPLE_POINT_2D), allocatable :: buffer(:)
        integer :: n
    end type SAMPLE_BUFFER_2D

    !> A sampling point, i.e., a tuple (i, x, f(x))
    type, public :: SAMPLE_POINT_3D
        integer(c_int64_t) :: i
        REAL_TYPE :: x(3)
        VALUE_TYPE :: fx
    end type SAMPLE_POINT_3D

    !> A buffer for the use of sampling points
    type, public :: SAMPLE_BUFFER_3D
        type(SAMPLE_POINT_3D), allocatable :: buffer(:)
        integer :: n
    end type SAMPLE_BUFFER_3D
    ...
    type, public :: SAMPLER_2D
        integer :: ncomp, nacc, npost
        REAL_TYPE :: beta, coeffs(2, 2)
        integer :: statsign(2)
    contains
        private
    ...
    type, public :: SAMPLER_3D
        integer :: ncomp, nacc, npost
        REAL_TYPE :: beta, coeffs(3, 3)
        integer :: statsign(3)
    contains
        private
    ...
    type, private, extends(SAMPLER) :: binning_sampler
        integer :: nbins
    contains
#ifdef VALUE_IS_COMPLEX
        procedure, non_overridable :: add_z => rhist_add
        procedure, non_overridable :: postprocess_z => rhist_postprocess
#else
    ...
    ! Sampler which just bins the data.
    type, private, extends(SAMPLER_2D) :: binning_sampler2d
        integer :: nbins(2)
    contains
#ifdef VALUE_IS_COMPLEX
        procedure, non_overridable :: add_z => rhist2d_add
        procedure, non_overridable :: postprocess_z => rhist2d_postprocess
#else
    ...
    ! Sampler which just bins the data.
    type, private, extends(SAMPLER_3D) :: binning_sampler3d
        integer :: nbins(3)
    contains
#ifdef VALUE_IS_COMPLEX
        procedure, non_overridable :: add_z => rhist3d_add
        procedure, non_overridable :: postprocess_z => rhist3d_postprocess
#else
    ...
    type, private, extends(SAMPLER) :: matsubara_sampler
    ...
    type, private, extends(SAMPLER_2D) :: matsubara_sampler_2d
    ...
    type, private, extends(SAMPLER) :: legendre_sampler
        integer :: nleg
    contains
#ifdef VALUE_IS_COMPLEX
        procedure, non_overridable :: add_z => legendre_add
        procedure, non_overridable :: postprocess_z => legendre_postprocess
#else
    ...
    type, extends(SAMPLER) :: nfft_sampler
        ...
        type(c_ptr) :: fft_plan
        COMPLEX_TYPE, allocatable :: fft_in(:,:)
        COMPLEX_TYPE, allocatable :: fft_out(:,:)
    contains
    ...
    type, extends(SAMPLER_2D) :: nfft_sampler2d
        integer, allocatable :: w1(:), w2(:)
        integer :: nextgrid(2)
    ...
    type, extends(SAMPLER_3D) :: nfft_sampler3d
...
#ifdef VALUE_IS_COMPLEX
    logical, parameter :: value_is_complex = .true.
#else
    logical, parameter :: value_is_complex = .false.
#endif

    REAL_TYPE, parameter :: PI = acos(-1.0D0)
...
contains
    ...
    pure subroutine p_prepare_buffer(this, n)
        type(SAMPLE_BUFFER), intent(inout) :: this
        integer, intent(in) :: n
    ...
    pure subroutine p_prepare_buffer2d(this, n)
        type(SAMPLE_BUFFER_2D), intent(inout) :: this
        integer, intent(in) :: n
    ...
    pure subroutine p_prepare_buffer3d(this, n)
        type(SAMPLE_BUFFER_3D), intent(inout) :: this
        integer, intent(in) :: n
    ...
    pure function sample_wraptau(this, sample) result(res)
        class(SAMPLER), intent(in) :: this
        type(SAMPLE_POINT), intent(in) :: sample
        type(SAMPLE_POINT) :: res
    ...
    pure function sample_2d_wraptau(this, sample) result(res)
        class(SAMPLER_2D), intent(in) :: this
        type(SAMPLE_POINT_2D), intent(in) :: sample
        type(SAMPLE_POINT_2D) :: res
        integer :: i
    ...
    pure function sample_3d_wraptau(this, sample) result(res)
        class(SAMPLER_3D), intent(in) :: this
        type(SAMPLE_POINT_3D), intent(in) :: sample
        type(SAMPLE_POINT_3D) :: res
        integer :: i
    ...
    subroutine p_sampler_add_d(this, sample, acc)
        ...
        type(SAMPLE_POINT), intent(in) :: sample(:)
        REAL_TYPE, intent(inout) :: acc(this%nacc, this%ncomp)
        type(SAMPLE_POINT) :: convention_sample(size(sample))
    ...
    subroutine p_sampler_add_z(this, sample, acc)
        class(SAMPLER), intent(inout) :: this
        type(SAMPLE_POINT), intent(in) :: sample(:)
        COMPLEX_TYPE, intent(inout) :: acc(this%nacc, this%ncomp)
        type(SAMPLE_POINT) :: convention_sample(size(sample))
    ...
    subroutine p_sampler_postprocess_d(this, acc, res)
        class(SAMPLER), intent(inout) :: this
        REAL_TYPE, intent(in), target :: acc(this%nacc, this%ncomp)
        REAL_TYPE, intent(out), target :: res(this%npost, this%ncomp)
    ...
    subroutine p_sampler_postprocess_z(this, acc, res)
        class(SAMPLER), intent(inout) :: this
        COMPLEX_TYPE, intent(in), target :: acc(this%nacc, this%ncomp)
        COMPLEX_TYPE, intent(out), target :: res(this%npost, this%ncomp)

        call this%postprocess_z(acc, res)
    end subroutine

    subroutine p_sampler_delete(this)
        class(SAMPLER), allocatable, intent(inout) :: this
    ...
    subroutine p_sampler2d_add_d(this, sample, acc)
        class(SAMPLER_2D), intent(inout) :: this
        type(SAMPLE_POINT_2D), intent(in) :: sample(:)
        REAL_TYPE, intent(inout), target :: acc(this%nacc, this%ncomp)
        type(SAMPLE_POINT_2D) :: convention_sample(size(sample))
        integer :: i
    ...
    subroutine p_sampler2d_add_z(this, sample, acc)
        class(SAMPLER_2D), intent(inout) :: this
        type(SAMPLE_POINT_2D), intent(in) :: sample(:)
        COMPLEX_TYPE, intent(inout), target :: acc(this%nacc, this%ncomp)
        type(SAMPLE_POINT_2D) :: convention_sample(size(sample))
        integer :: i
    ...
    subroutine p_sampler2d_delete(this)
        class(SAMPLER_2D), allocatable, intent(inout) :: this
    ...
    subroutine p_sampler3d_add_d(this, sample, acc)
        class(SAMPLER_3D), intent(inout) :: this
        type(SAMPLE_POINT_3D), intent(in) :: sample(:)
        REAL_TYPE, intent(inout), target :: acc(this%nacc, this%ncomp)
        type(SAMPLE_POINT_3D) :: convention_sample(size(sample))
        integer :: i
    ...
    subroutine p_sampler3d_delete(this)
        class(SAMPLER_3D), allocatable, intent(inout) :: this
    ...
    subroutine sampler_init(this, ncomp, nacc, npost, beta, statsign, coeff)
        ...
        REAL_TYPE, intent(in) :: beta, coeff
    ...
    subroutine sampler2d_init(this, ncomp, nacc, npost, beta, statsign, coeffs)
        ...
        REAL_TYPE, intent(in) :: beta, coeffs(2, 2)
    ...
    subroutine sampler3d_init(this, ncomp, nacc, npost, beta, statsign, coeffs)
        ...
        REAL_TYPE, intent(in) :: beta, coeffs(3, 3)
    ...
    subroutine rhist_init(this_s, ncomp, beta, statsign, nbins, coeff)
        class(SAMPLER), allocatable, intent(inout) :: this_s
    ...
    subroutine rhist_add(this, sample, acc)
        ...
        type(SAMPLE_POINT), intent(in) :: sample(:)
        VALUE_TYPE, intent(inout) :: acc(0:this%nacc-1, this%ncomp)

        REAL_TYPE :: invd
        integer(c_int64_t) :: i, bin
        type(SAMPLE_POINT) :: curr
    ...
    subroutine rhist_postprocess(this, acc, res)
        class(binning_sampler), intent(inout) :: this
        VALUE_TYPE, intent(in), target :: acc(this%nacc, this%ncomp)
        VALUE_TYPE, intent(out), target :: res(this%npost, this%ncomp)
    ...
    subroutine rhist2d_init(this_s, ncomp, beta, statsign, nbins, coeffs)
        class(SAMPLER_2D), allocatable, intent(inout) :: this_s
    ...
    subroutine rhist2d_add(this, sample, acc)
        ...
        type(SAMPLE_POINT_2D), intent(in) :: sample(:)
        VALUE_TYPE, intent(inout), target :: acc(0:this%nacc-1, this%ncomp)
        VALUE_TYPE, pointer :: buf(:, :, :)

        REAL_TYPE :: invd(2)
        integer(c_int64_t) :: i, bin(2)
        type(SAMPLE_POINT_2D) :: curr
    ...
    subroutine rhist3d_init(this_s, ncomp, beta, statsign, nbins, coeffs)
        class(SAMPLER_3D), allocatable, intent(inout) :: this_s
    ...
    subroutine nfourier2d_init(this_s, ncomp, beta, statsign, niw, coeff)
        ...
        REAL_TYPE, intent(in) :: beta, coeff(2,2)
    ...
    subroutine nfourier2d_add(this, sample, acc)
        ...
        type(SAMPLE_POINT_2D), intent(in) :: sample(:)
        COMPLEX_TYPE, intent(inout), target :: acc(this%nacc, this%ncomp)
        COMPLEX_TYPE, pointer :: buf(:, :, :)

        integer(c_int64_t) :: i, n1, n2
        COMPLEX_TYPE :: val, expfact1, expfact2
    ...
    subroutine legendre_add(this, sample, acc)
        ...
        VALUE_TYPE :: p0, p1, pnext, scaledx
        integer(c_int64_t) :: i, n
        type(SAMPLE_POINT) :: curr
    ...
    subroutine legendre_postprocess(this, acc, res)
        class(legendre_sampler), intent(inout) :: this
        VALUE_TYPE, intent(in), target :: acc(0:this%nacc-1, this%ncomp)
        VALUE_TYPE, intent(out), target :: res(0:this%npost-1, this%ncomp)
    ...
    subroutine nfft_foldback(this, acc, out)
        ...
        COMPLEX_TYPE, intent(in) :: acc(this%nextgrid, this%ncomp)
        COMPLEX_TYPE, intent(out) :: out(this%window%ngrid, this%ncomp)
    ...
    subroutine dft_naive(this, res)
        class(nfft_sampler), intent(in) :: this
        COMPLEX_TYPE, intent(out) :: res(this%npost, this%ncomp)

        integer :: k, j
        REAL_TYPE :: twopi_n
        COMPLEX_TYPE :: expfact
    ...
    subroutine nfft2d_foldback(this, acc, out)
        ...
        COMPLEX_TYPE, intent(in), target :: acc(this%nacc, this%ncomp)
        COMPLEX_TYPE, intent(out) :: out(this%window(1)%ngrid,&
                                         this%window(2)%ngrid,&
                                         this%ncomp)
        COMPLEX_TYPE, pointer :: acc2d(:, :, :)
    ...
    subroutine dft2d_naive(this, res)
        class(nfft_sampler2d), intent(in) :: this
        COMPLEX_TYPE, intent(out) :: res(size(this%w1), size(this%w2), this%ncomp)

        integer :: k, j, l, m
        REAL_TYPE :: twopi_n(2)
        COMPLEX_TYPE :: expfact(2)
    ...
    subroutine dft3d_naive(this, res)
        class(nfft_sampler3d), intent(in) :: this
        COMPLEX_TYPE, intent(out) :: res(size(this%w1), size(this%w2),&
                                         size(this%w3), this%ncomp)

        integer :: k, j, l, m, n, p
        REAL_TYPE :: twopi_n(3)
        COMPLEX_TYPE :: expfact(3)
    ...
    subroutine nfft3d_inv_conv(this, res)
        ...
        REAL_TYPE :: phihat1, phihat2, phihat3
    ...
    subroutine gauss_add_slow(this, acc, pos, delta, fx)
        type(gauss_window), intent(in) :: this
        COMPLEX_TYPE, intent(inout) :: acc(:)
        REAL_TYPE, intent(in) :: delta
        VALUE_TYPE, intent(in) :: fx
    ...
    subroutine gauss_add3d_fast(this, acc, pos, delta, fx)
        ...
        integer :: l1, l2, l3
        COMPLEX_TYPE :: val(3), valprod(2:3)
        REAL_TYPE :: step(3)
