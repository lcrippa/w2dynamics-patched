...
class MaxEntBase:
    ...
    the Fortran library call is wrapped by the method self.computeSpec
    ...
    def __init__(self,kernelMode,beta,Ncorr,NGrid1,NGrid2,NGrid3,\
        ...
        self.lib = ctypes.CDLL(pathlib.Path(__file__).parent / "libMAXENT.so")
        self.lib.w2maxent.argtypes = [ctypes.POINTER(ctypes.c_double),
                                    ctypes.POINTER(ctypes.c_double),
                                    ctypes.POINTER(ctypes.c_double),
                                    ctypes.c_int32,
                                    ctypes.c_void_p,  # double complex
                                    ctypes.POINTER(ctypes.c_double),
                                    ctypes.POINTER(ctypes.c_double),
                                    ctypes.c_int32,
                                    ctypes.c_double,
                                    ...
                                    ctypes.c_int32,
                                    ctypes.c_char_p]
        self.lib.w2maxent.restype = None
        self.lib.normalizeToGrid.argtypes = [ctypes.POINTER(ctypes.c_double),
                                           ctypes.POINTER(ctypes.c_double),
                                           ctypes.c_int32,
                                           ctypes.c_int32]
        self.lib.normalizeToGrid.restype = None
        ...
        #   smoothSpectrum(omega_i)=sum(spectralFunction(k:l))/(l-k+1),
        #                               k=omega_i-smoothCnt, l=omega_i+smoothCnt,
        #1: smoothing by locally integrate spectral function:
        #   smoothSpectrum(omega_i)=SUM(domega(k:l)*spectralFunction(k:l))/SUM(domgea(k:l),
        #   important for non-equidistant grid-points
    ...
    def computeGrid(self):
        ...
        self.grid[:self.NGrid1] = \
                np.linspace(self.wmin,self.w1,num=self.NGrid1,endpoint=False)
        self.grid[self.NGrid1:self.NGrid1+self.NGrid2] = \
                np.linspace(self.w1,self.w2,num=self.NGrid2,endpoint=True)
        self.grid[-self.NGrid3:] = \
                np.linspace(self.wmax,self.w2,num=self.NGrid3,endpoint=False)[::-1]

    def allocateArrays(self):
        #arrays in input data points
        self.corrGrid = np.zeros(self.Ncorr,dtype=np.double,order='F')
        self.corrCov = np.zeros(self.Ncorr,dtype=np.double,order='F')
        self.corrReal = np.zeros(self.Ncorr,dtype=np.double,order='F')
        self.corrImag = np.zeros(self.Ncorr,dtype=np.double,order='F')
        self.corr = np.zeros(self.Ncorr,dtype=np.cdouble,order='F')

        #arrays in output points
        self.grid = np.zeros(self.NGrid,dtype=np.double,order='F')
        self.model = np.zeros(self.NGrid,dtype=np.double,order='F')
        self.spec = np.zeros(self.NGrid,dtype=np.double,order='F')
        self.gws = np.zeros(self.NGrid,dtype=np.cdouble,order='F')

    def computeSpec(self):
        self.lib.w2maxent(self.spec.ctypes
                            .data_as(ctypes.POINTER(ctypes.c_double)),
                            self.model.ctypes
                            .data_as(ctypes.POINTER(ctypes.c_double)),
                            self.grid.ctypes
                            .data_as(ctypes.POINTER(ctypes.c_double)),
                            ctypes.c_int32(self.spec.shape[-1]),
                            self.corr.ctypes
                            .data_as(ctypes.c_void_p),
                            self.corrCov.ctypes
                            .data_as(ctypes.POINTER(ctypes.c_double)),
                            self.corrGrid.ctypes
                            .data_as(ctypes.POINTER(ctypes.c_double)),
                            ctypes.c_int32(self.corr.shape[-1]),
                            ctypes.c_double(self.alpha),
                            ctypes.c_int32(self.kernelMode),
                            ctypes.c_double(self.beta),
                            ctypes.c_int64(self.NMonteCarlo),
                            ctypes.c_int64(self.NAnnealing),
                            ctypes.c_int32(self.seed),
                            ctypes.c_int32(self.NSmooth),
                            ctypes.c_int32(self.smoothCnt),
                            ctypes.c_int32(self.smoothingMethod),
                            ctypes.c_char_p(self.logprefix.encode()))

class Green2Aw(MaxEntBase):
    """
    this derived class is used for the continuation of a greens function in tau
    (kernel mode 0) and iw (kernel mode 1)
    ...
    def symmetrize(self):
        self.corrReal[:self.corrReal.shape[0]//2] = 0.5*(
                self.corrReal[:self.corrReal.shape[0]//2]
                + self.corrReal[self.corrReal.shape[0]//2:])
        self.corrReal[self.corrReal.shape[0]//2:] = \
                self.corrReal[:self.corrReal.shape[0]//2]
        self.corrCov[:self.corrCov.shape[0]//2] = 0.5*(
    ...
    def computeModel(self):
        self.model[:] = 1.
        assert len(self.model.shape) == 1
        assert self.model.shape == self.grid.shape
        self.lib.normalizeToGrid(self.model.ctypes
                               .data_as(ctypes.POINTER(ctypes.c_double)),
                               self.grid.ctypes
                               .data_as(ctypes.POINTER(ctypes.c_double)),
                               ctypes.c_int32(self.grid.shape[-1]),
                               ctypes.c_int32(self.kernelMode))
...
class Chi2Aw(MaxEntBase):
    """
    this derived class is used for the continuation of bosonic chi function in
    tau (kernel mode 10 or 11)
    ...
    def printSpec(self,fname):
        ...
        for i in range(self.NGrid):
            ...
            print(self.grid[i], self.spec[i]*kernelModeFactor, self.spec[i],
                  self.model[i], file=aw)

    def printGws(self, fname):
        gws = open(fname, "w")
        print("# w      Re[Chi(w)]      Im[Chi(w)]      model(w)", file=gws)
        for i in range(self.NGrid):
            print(self.grid[i], self.gws[i].real, self.gws[i].imag,
                  self.model[i], file=gws)
