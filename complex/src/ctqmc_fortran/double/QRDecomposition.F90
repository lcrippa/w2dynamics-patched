...
module MQRDecompositionD
    ...
    !! are the factors for the reflectors (output of DGEQRF). If pivoting is
...
contains
    ...
    pure function conjg_rot(a) result(r)
        type(DPlaneRotation), intent(in) :: a
        type(DPlaneRotation) :: r
    ...
    function qr_det_pivot(qr, tau, jpiv) result(det)
        real(c_double), intent(in) :: qr(:, :), tau(:)
    ...
    subroutine p_qr_hessenberg(k, h, ldh, g)
        ...
        type(DPlaneRotation), intent(out) :: g(:, :)

        integer :: i, j, l, m, n
        type(DPlaneRotation) :: gcurr
        ...
        do j = 1, n - 1
            do l = k, 1, -1
                ...
                if (i >= m) then
                    ...
                    call DROT(n-j, h(i, j+1), ldh, h(i+1, j+1), ldh, &
                             gcurr%c, gcurr%s)
                    g(l, j) = gcurr
                endif
            enddo
    ...
    subroutine p_rotate_q(q, ldq, g)
        ...
        type(DPlaneRotation), intent(in) :: g(:, :)

        type(DPlaneRotation) :: gadj
        integer :: i, j, k, l, m, n
        ...
        do j = 1, n - 1
            do l = k, 1, -1
                ...
                gadj = conjg(g(l, j))
                call DROT(m, q(1, i), 1, q(1, i+1), 1, gadj%c, gadj%s)
            enddo
        enddo
    end subroutine
    ...
    subroutine p_q_diagonalize(q, ldq, giv)
        integer, intent(in) :: ldq
        real(c_double), intent(inout) :: q(:, :)
        type(DPlaneRotation), intent(out) :: giv(:, :)
        ...
        do d = n-1, 1, -1
            do i = 1, m
                ...
                if (j >= n) then
                    ...
                    if (i < m) then
                        call DROT(m - i, q(i+1, j), 1, q(i+1, j+1), 1, &
                                 gadj%c, gadj%s)
                    endif
                    giv(i, d) = conjg(gadj)
                endif
    ...
    subroutine p_rotate_r(r, ldr, giv)
        ...
        type(DPlaneRotation), intent(in) :: giv(:, :)
        ...
        do d = n-1, 1, -1
            do i = 1, k
                ...
                call DROT(m, r(j, 1), ldr, r(j+1, 1), ldr, gcurr%c, gcurr%s)
            enddo
        enddo
    end subroutine

    subroutine rotate_inplace(f, g, rot)
        real(c_double), intent(inout) :: f, g
        type(DPlaneRotation), intent(out) :: rot
        real(c_double) :: r

        call DLARTG(f, g, rot%c, rot%s, r)
        f = r
        g = 0
    end subroutine
