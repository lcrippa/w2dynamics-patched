...
module ZMOperator
    ...
    type :: TSubOperator
        complex(c_double_complex), allocatable :: op(:,:)
    ...
    type :: TSubEigen
        ...
        complex(c_double_complex), allocatable :: evec(:,:)
    ...
    type, public :: ZTStateVector
        ...
        complex(c_double_complex), allocatable :: vec(:,:)
                                ! buffer (potentially larger)
    end type ZTStateVector

    type, public, extends(ZTStateVector) :: ZTKet
    end type ZTKet

    type, public, extends(ZTStateVector) :: ZTBra
    end type ZTBra
    ...
    complex(c_double_complex), parameter :: ONE = 1, ZERO = 0
...
contains
    ...
    pure function p_is_initialized(self) result(test)
        type(ZTOperator), intent(in) :: self
        logical :: test
    ...
    subroutine p_init_operator_sizes(self, block_size, connect)
        type(ZTOperator), intent(out) :: self
    ...
    subroutine p_dest_operator(self)
        type(ZTOperator), intent(inout) :: self
    ...
    subroutine p_oper_set_block(self, i, j, val)
        ...
        complex(c_double_complex), intent(in) :: val(0:, 0:)
    ...
    subroutine p_oper_get_block(self, i, j, op)
        type(ZTOperator), intent(in) :: self
        integer, intent(in) :: i, j
        complex(c_double_complex), allocatable, intent(inout) :: op(:,:)
        integer :: nsstates
    ...
    function p_mul_opers(x, y) result(r)
        type(ZTOperator), target, intent(in) :: x, y
        type(ZTOperator) :: r
        integer :: i, nsstates, rconn, isize
        complex(c_double_complex), pointer :: xop(:, :), yop(:, :)
    ...
    subroutine p_add_to_oper(r, term, scale)
        type(ZTOperator), intent(inout) :: r
        type(ZTOperator), intent(in) :: term
        complex(c_double_complex) :: scale
        integer :: i, nsstates
    ...
    subroutine p_diag_Operator(this, Eigen, epsevec)
        type(ZTOperator), intent(in) :: this
        type(ZOperEigen), intent(out) :: Eigen
        real(c_double), intent(in), optional :: epsevec
    !output
    !local
        complex(c_double_complex), allocatable :: c_WORK(:)
    ...
    subroutine p_force_hermitian(H)
        type(ZTOperator), intent(inout) :: H
        integer :: sst
    ...
    subroutine process_eigenvectors(vecs, epsevec)
        complex(c_double_complex), intent(inout) :: vecs(0:, 0:)
    ...
    subroutine p_start_sst_list(self, valid_out, nvalid_out)
        type(ZOperEigen), intent(in) :: self
    ...
    subroutine p_transform_inv_eigenbasis(eigen, op)
        type(ZOperEigen), intent(in) :: Eigen
        type(ZTOperator), intent(inout) :: op
        integer :: i, j, nmax
    ...
    subroutine p_transform_inv_eigenbasis_mat(op, lbasis, rbasis, work)
        complex(c_double_complex), intent(inout) :: op(:,:), work(:,:)
        complex(c_double_complex), intent(in) :: lbasis(:,:), rbasis(:,:)
        integer :: m, n
    ...
    subroutine recompute_egs(self)
        type(ZOperEigen), intent(inout) :: self
        integer :: isst
    ...
    pure subroutine t_swap(x, y)
        class(ZTStateVector), intent(inout) :: x, y
    ...
    pure subroutine t_swap_ket(x, y)
        type(ZTKet), intent(inout) :: x, y

        call t_swap(x, y)
    end subroutine

    pure subroutine t_swap_bra(x, y)
        type(ZTBra), intent(inout) :: x, y

        call t_swap(x, y)
    end subroutine

    pure subroutine swap_vec(x, y)
        complex(c_double_complex), allocatable, intent(inout) :: x(:,:), y(:,:)
    ...
    subroutine t_set(self, sst, val, exp)
        class(ZTStateVector), intent(inout) :: self
        integer, intent(in) :: sst
    ...
    subroutine t_set_real(self, sst, val, exp)
        ...
        complex(c_double_complex) :: val_complex(size(val))
    ...
    subroutine t_copy_ket(state_in, state_out)
        class(ZTKet), intent(in) :: state_in
        class(ZTKet), intent(inout) :: state_out

        call t_copy(state_in, state_out)
    end

    subroutine t_copy_bra(state_in, state_out)
        class(ZTBra), intent(in) :: state_in
        class(ZTBra), intent(inout) :: state_out

        call t_copy(state_in, state_out)
    end

    subroutine t_copy(state_in, state_out)
        class(ZTStateVector), intent(in) :: state_in
        class(ZTStateVector), intent(inout) :: state_out
    ...
    pure elemental integer function state_get_sst(state) result(sst)
        class(ZTStateVector), intent(in) :: state
    ...
    function t_braket_dotprod(b, k) result(bk)
        ...
        if (b%sst /= k%sst) then
            ...
            braket = ZERO
            do i = 1, k%nvec
                braket = braket + ZDOTC(b%sstsize, b%vec(1, i), 1, k%vec(1, i), 1)
            enddo
    ...
    pure subroutine t_time_evolve(H_eigen, dtau, state)
        type(ZOperEigen), intent(in) :: H_eigen
        real(c_double), intent(in) :: dtau
        class(ZTStateVector), intent(inout) :: state
    ...
    function ket_trace_boltzmann(H_eigen, beta, state) result(tr)
        type(ZOperEigen), target, intent(in) :: H_eigen
        real(c_double), intent(in) :: beta
        type(ZTKet), intent(in) :: state
        type(ZExtRange) :: tr
    ...
    function state_trace_boltzmann(H_eigen, beta, state) result(tr)
        ...
        type(ZExtRange) :: tr
        real(c_double) :: E0
        complex(c_double_complex) :: tr_diff
    ...
    subroutine t_apply_oper(vout, op, vin)
        type(ZTKet), intent(inout) :: vout
        type(ZTOperator), intent(in), target :: op
        type(ZTKet), intent(in) :: vin
        complex(c_double_complex), pointer :: block_op(:,:)
        ...
        if (vout%sst == -1) then
            ...
            call ZGEMM('N', 'N', vout%sstsize, vout%nvec, vin%sstsize, ONE, &
    ...
    subroutine t_apply_adjoint(vout, op, vin)
        type(ZTBra), intent(inout) :: vout
        type(ZTOperator), intent(in), target :: op
        type(ZTBra), intent(in) :: vin
        complex(c_double_complex), pointer :: block_op(:,:)
        ...
        if (vout%sst == -1) then
            ...
            call ZGEMM('C', 'N', vout%sstsize, vout%nvec, vin%sstsize, ONE, &
    ...
    subroutine t_eigenstate_in_eigenbasis(stvec, eigen, isst, ist)
        class(ZTStateVector), intent(inout) :: stvec
        type(ZOperEigen), intent(in) :: eigen
    ...
    subroutine t_ket_set_operator(self, op, sst_in)
        type(ZTKet), intent(inout) :: self
        type(ZTOperator), intent(in) :: op
    ...
    subroutine t_outer_integral(op, b, k, tau, H, alpha, work)
        type(ZTOperator), intent(inout) :: op, work
        type(ZTKet), intent(in) :: k
        type(ZTBra), intent(in) :: b
        real(c_double), intent(in) :: tau
        type(ZOperEigen), intent(in) :: H
        type(ZExtRange), intent(in) :: alpha

        if (k%sst /= b%sst) &
            error stop 'Ket and bra must belong to the same block for now'

        !alpha_x = alpha * ZExtRange(exp(DExtRange(-tau * H%egs)))
    ...
    subroutine t_outer_product(op, b, k, alpha, gamma)
        type(ZTOperator), intent(inout), target :: op
        type(ZTKet), intent(in) :: k
        type(ZTBra), intent(in) :: b
        type(ZExtRange), intent(in) :: alpha
        complex(c_double_complex), intent(in) :: gamma
        complex(c_double_complex) :: alpha_d
        complex(c_double_complex), pointer :: subop(:,:)
    ...
    subroutine state_assert_close(a, b, rtol)
        class(ZTStateVector), intent(in) :: a, b
        real(c_double), intent(in) :: rtol
        character(len=70) :: reason

        real(c_double) :: alpha
        complex(c_double_complex) :: diff(a%sstsize, a%nvec)
