...
module MExtRangeZ
    ...
    type, public :: ZExtRange
        private
        complex(c_double_complex) :: val = 1
...
contains
    ...
    pure elemental function p_add(x, y) result(r)
        type(ZExtRange), intent(in) :: x, y
        type(ZExtRange) :: r
        complex(c_double_complex) :: xval, yval
    ...
    pure subroutine p_rescale(x, y, xval, yval, expon)
        type(ZExtRange), intent(in) :: x, y
        complex(c_double_complex), intent(out) :: xval, yval
    ...
    pure subroutine p_split(val, fract, expon)
        complex(c_double_complex), intent(in) :: val
        complex(c_double_complex), intent(out) :: fract
        integer(c_long), intent(out) :: expon
        intrinsic :: fraction, exponent
        integer :: re_exp, im_exp
        real(c_double) :: r_val_real, r_val_imag
    ...
    pure logical function p_isclose(a, b, rtol) result(result)
        type(ZExtRange), intent(in) :: a, b
        real(c_double), intent(in), optional :: rtol
        complex(c_double_complex) :: aval, bval
        integer(c_long) :: expon
