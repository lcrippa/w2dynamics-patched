...
module MPsiZ
    ...
    type, public :: ZTPsis
        type(ZTOperator), allocatable :: Psis(:,:,:)
    end type
...
contains
    ...
    subroutine p_init_operator(this, dstates)
        type(ZTOperator), intent(out) :: this
    ...
    subroutine p_make_hamiltonian(muimp, u_matrix, states, psi, hloc, qnthr)
        complex(c_double_complex), intent(in) :: muimp(:, :, :, :), u_matrix(:, :, :, :)
        real(c_double), intent(in) :: qnthr
        type(TStates), intent(inout) :: states
        type(ZTPsis), intent(inout) :: psi
        type(ZTOperator), intent(out) :: hloc
        type(TQNSpec) :: qnspec
    ...
    subroutine add_u_hamiltonian(u_matrix, h, dpsis)
        ...
        complex(c_double_complex) :: u_value
        type(ZTOperator) :: op12, op124, op1243
    ...
    subroutine p_transform_Psis(Eigen, transformed_Psis)
        ...
        type(ZTPsis), intent(inout) :: transformed_Psis
        integer :: iB, iS, CA
    ...
    function p_rho1_from_density_matrix(rho_mb, hloc, psis) result(rho1)
        type(ZTOperator), intent(in) :: rho_mb
        type(ZOperEigen), intent(in) :: hloc
        type(ZTPsis), intent(in) :: psis
        complex(c_double_complex), allocatable :: rho1(:, :)

        integer :: ib2, is2, ib1, is1, iSSt
        integer :: nsstates, nbands

        type(ZTKet) :: ket1, ket2
        type(ZTBra) :: bra_rho
        type(ZTOperator) :: rho_dag
        complex(c_double_complex) :: rho_entry
    ...
    function p_double_occ_from_density_matrix(rho_mb, hloc, psis) result(occ)
        ...
        type(ZOperEigen), intent(in) :: hloc
        type(ZTPsis), intent(in) :: psis
        complex(c_double_complex), allocatable :: occ(:, :, :, :)

        integer :: ib2, is2, ib1, is1, iSSt
        integer :: nsstates, nbands

        type(ZTKet) :: ket, ket1, ket2, ketw
        type(ZTBra) :: bra_rho
        type(ZTOperator) :: rho_dag
        complex(c_double_complex) :: rho_entry
