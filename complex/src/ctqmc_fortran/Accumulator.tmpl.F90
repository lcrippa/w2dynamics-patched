...
contains
    ...
    ! ######## METHODS FOR ANY ACCUMULATOR ########

    subroutine p_accumulator_init(this, typ, ncomp, nlevels)
        type(ACCUMULATOR), intent(inout) :: this
    ...
    subroutine p_accumulator_buffer(this, buf)
        type(ACCUMULATOR), intent(inout), target :: this
        VALUE_TYPE, intent(out), pointer :: buf(:)
    ...
    function p_accumulator_num_comp(this) result(answer)
        type(ACCUMULATOR), intent(in) :: this
...
#else
        atom%sqdiff = atom%sqdiff + incr1 * incr2
#endif
    end subroutine
