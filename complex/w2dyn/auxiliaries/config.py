...
def lattice_from_cfg(cfg):
    ...
    #GS:
        ...
        # beta is needed for the Matsubara frequencies, in case leadsiw needs to be constructed
        beta = cfg["General"]["beta"]
        hkfile = open(cfg["General"]["HkFile"], "r")
...
def interaction_from_cfg(atom_cfg, cfg):
    ...
    if int_type == "Density":
        if Uw == 1:
            _Retarded2Shifts = \
                dynamicalU.Replace_Retarded_Interaction_by_a_Shift_of_instantaneous_Potentials( \
                        0, zero_umatrix, Uw_Mat, 1)
        ...
        if Uw == 1:
            _Retarded2Shifts = \
                dynamicalU.Replace_Retarded_Interaction_by_a_Shift_of_instantaneous_Potentials( \
                    0, zero_umatrix, Uw_Mat,1)
            orb_u_matrix = \
                _Retarded2Shifts.shift_instantaneous_densitydensity_potentials_from_ReadNormalUmatrix_config(\
                    orb_u_matrix)
...
def atomlist_from_cfg(cfg, norbitals=None):
    ...
    for atom_cfg in cfg["Atoms"].values():
        ...
        # read H_imp file
        himp = None
        if atom_cfg["HimpFile"] is not None:
            with open(atom_cfg["HimpFile"], "r") as himp_file:
                himp = _input.read_himp_autoformat(himp_file, nds)
...
def doublecounting_from_cfg(cfg, ineq_list, mylattice, atom_list, u_full):
    ...
    if isinstance(dc_type, list):
        ...
        return doublecounting.Wterm(np.array(cfg["General"]["Wterm_U2"],dtype=np.double),
                                    ...
                                    np.array(cfg["General"]["Wterm_a22"],dtype=np.double),
                                    np.array(cfg["General"]["Wterm_a11"],dtype=np.double),
