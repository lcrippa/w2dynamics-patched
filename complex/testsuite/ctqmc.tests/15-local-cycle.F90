...
program test
   ...
   type(TProposalWeights), parameter :: prop_weights = TProposalWeights( &
            TauShiftMove = 0.01d0, &
            GlobalMove = 0.01d0, &
            WormInsert = 0.0d0, &
            WormReplace = 0.0d0, &
            OuterMove = 0.1d0, &
            FourOperatorMove = 0.1d0 &
            )
   ...
   do i = 1, 100
      ...
      addloop: do
         ...
         if (.not. is_update_allowed(addmove%local)) &
            cycle addloop
         call compute_weight(addmove%local)
         call compute_weight(addmove%bath)
      ...
      add4loop: do
         ...
         if (.not. is_update_allowed(add4move%local)) &
             cycle add4loop
         call compute_weight(add4move%local)
         call compute_weight(add4move%bath)
      ...
      remloop: do
         ...
         if (.not. is_update_allowed(remmove%local)) &
            cycle remloop
         call compute_weight(remmove%local)
         call compute_weight(remmove%bath)
      ...
      rem4loop: do
         ...
         if (.not. is_update_allowed(rem4move%local)) &
            cycle rem4loop
         call compute_weight(rem4move%local)
         call compute_weight(rem4move%bath)
      ...
      shiftloop: do
         ...
         if (.not. is_update_allowed(shiftmove%local)) &
            cycle shiftloop

         call compute_weight(shiftmove%local)
         call compute_weight(shiftmove%bath)
      ...
      permloop: do
         ...
         if (.not. is_update_allowed(permmove%local)) &
            cycle permloop
         if (.not. is_update_allowed(permmove%bath)) &
            cycle permloop

         call compute_weight(permmove%local)
         call compute_weight(permmove%bath)
    ...
    subroutine test_ctqmc_sample_steps(Nsteps, allowed_worm_sector, iComponent, samprng)
        ...
        do i = 1, Nsteps
            ...
            call perform_move( &
                    ...
                    prop_weights, wormEta(allowed_worm_sector), &
