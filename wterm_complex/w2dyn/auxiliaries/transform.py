...
def hybr_from_sites(epsk, vki, ffreq, tau, beta=None):
    ...
    Parameters:
      - epsk     list of bath energies or bath Hamiltonian matrix
    ...
    if epsk.ndim == 2:
        epsk, epsvecs = np.linalg.eigh(epsk)
        vki = np.matmul(epsvecs.T.conj(), vki)
    elif epsk.ndim != 1:
        raise ValueError("epsk must have one or two dimensions")

    # vijk[i, j, k] = (V^\dag)_{ik} V_{kj}: (nflav, nflav, nbath)
    vijk = vki.T.conj()[:, None, :] * vki.T[None, :, :]
    # iwn[:, None] - epsk: (nfreq, nbath); vijk/...: (nflav, nflav, nfreq, nbath)
