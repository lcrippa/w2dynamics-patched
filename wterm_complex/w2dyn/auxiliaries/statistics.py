...
class DistributedSample:
    ...
    def __init__(self, local, mpi_comm, ntotal=None):
        ...
        self.have_empty_ranks = (
            self.mpi_comm.allreduce(self.nlocal, op=mpi.MIN) == 0
        )

        if self.have_empty_ranks:
            # determine a rank with data
            self.refrank = self.mpi_comm.allreduce(
                self.mpi_comm.Get_rank() + 1 if self.nlocal > 0 else 0,
                op=mpi.MAX
            ) - 1
            if self.refrank < 0:
                raise ValueError("Not a single rank with data")

            target_local_shape = self.mpi_comm.bcast(self.local.shape,
                                                     root=self.refrank)
            target_local_dtype = self.mpi_comm.bcast(self.local.dtype,
                                                     root=self.refrank)
            if self.nlocal == 0:
                target_local_shape = (0,) + target_local_shape[1:]
                self.local = np.zeros(target_local_shape,
                                      dtype=target_local_dtype)
        ...
        if ntotal is not None:
            ...
            self.ntotal = self._reduce_inplace(self.ntotal)
    ...
    def _reduce_inplace(self, arr):
        ...
        if self.have_empty_ranks:
            target_shape = self.mpi_comm.bcast(arr.shape, root=self.refrank)
            target_dtype = self.mpi_comm.bcast(arr.dtype, root=self.refrank)
            if self.nlocal == 0:
    ...
    def sum(self):
        ...
        result = self._reduce_inplace(result)
        return result
    ...
    def _sqdiff(self, mean, dtype=float):
        ...
        sqdiff = self._reduce_inplace(sqdiff)
        return sqdiff
    ...
    def _bidiff(self, mean, dtype=float):
        ...
        diff = np.reshape(diff,
                          mean.shape * 2)
        diff = self._reduce_inplace(diff)
        return diff
