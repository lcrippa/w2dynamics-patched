    ...
    def set_siws(self, siw_dd=None, smom_dd=None, dc_full=None, init=False,
        ...
        if siw_dd is None:
            ...
            for ineq in self.ineq_list:
                siw_block = np.zeros((self.niwf, ineq.nd, nspins, ineq.nd,
                                      nspins), np.cdouble)
                ###TODOcompl: this is not general enough any more!
                #if not self.paramag:
        ...
        if smom_dd is None:
            warn("extracting moments from Sigma. Better use siw_mom",
                 UserWarning, 2)
            ###TODOcompl: here only real part
            smom_dd = [siw_block[:1].real for siw_block in siw_dd]

        # Enforce paramagnetic option
        ### TODOcompl: this is not yet existing for complex t_ij
        #if self.paramag:
            #siw_dd = [orbspin.symm_spins_matsubara(siw_block) for siw_block in siw_dd]
            #smom_dd = [orbspin.symm_spins_scalar(smom_block) for smom_block in smom_dd]
...
class CurveFitter:
    def __init__(self, totdens, startat, minhistory, maxhistory, log=None):
        self.totdens = totdens
        self.count = 0
    ...
    def _refit(self):
        if self.mus.shape[0] < self.minhistory:
            return
        ...
        results.append(opt.curve_fit(polyfun,
                                     ...
                                     p0=self.coeffs,
                                     sigma=(self.nerrs
                                            if not np.any(np.isnan(self.nerrs))
                                            else None),
                                     absolute_sigma=True,
                                     jac=jacobian))
    ...
    def has_mu(self):
        return ((self.mus.shape[0] >= self.minhistory)
                and (self.count >= self.startat))

    def next_mu(self):
        self._refit()
        nextmu = (self.totdens - self.coeffs[0]) / self.coeffs[1]
        return (nextmu
                if np.isfinite(nextmu)
                else self.mus[-1])
