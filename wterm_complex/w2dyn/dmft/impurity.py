...
class DummyMpiCommunicator:
    ...
    def bcast(self, rank_result, root): return rank_result
...
class ImpurityResult:
    ...
    def postprocessing(self, siw_method="dyson", smom_method="estimate"):
        r"""Set self-energy `self.siw` and its moment `self.smom`.
        ...
        siw_method : {'dyson', 'improved_worm', 'symmetric_improved_worm'}, optional
            ...
            or from symmetric improved estimators:
            `(\Sigma_H + \theta) / (1 + \mathcal{G}(\Sigma_H + \theta))`
    ...
    def calculate_siw(self, method):
        r"""Calculate the self-energy from impurity data.
