...
MODULE MMaximumEntropy
  ...
  REAL(c_double)      :: bayesianConvergence         = 0.05  ! numerical accuracy for the Bayesian convergence criterion [1] Eq. (4.28): {-2alpha*S / Tr[...] - 1 < bayesianConvergence } ?
  REAL(c_double)      :: bayesianUpdateAlpha         = 0.05  ! update factor for  Bayesian parameter alpha, see subroutine bayesianInverence
  !*** PARAMETER FOR BAYESIAN INVERENCE METHODE IN SUBROUTINE <bayesianInverence>

  !*** Metropolis and annealing parameters for determination of spectral function via Monte Carlo
  REAL(c_double)      :: noiseFilter                 = 1D-1  ! noiseFilter, only variations above the noise {noiseFilter*max(A(omega))} are considered as important for try and acceptance rate in MonteCarlo run

  REAL(c_double)      :: variationUpdate             = 1.5   ! magnitude for important sampling: varSpecFunc(omega) = variationStrength * noiseFilter *(spectralFunction(omega) + noiseFilter * max(spectralFunction(omega))):
                                                     ! variationStrength' = variationStrength * variationUpdate, methode by Sandivk
  REAL(c_double)      :: criticaleAcceptRate         = 1D-1  ! critical parameter for performing update of variationStrength with respect to try/acceptance-rate in MonteCarlo Algorithm
  REAL(c_double)      :: criticaleVariationInc       = 1D-2  ! critical parameter for how to update variational-strength-parameter
  REAL(c_double)      :: criticaleVariationDec       = 1D-3  ! critical parameter for how to update variational-strength-parameter

  REAL(c_double)      :: variationStrengthWarmpup    = 1.    ! initial variational magnitude for MonteCarlo search of most probable spectrum in the warm-up-run (starting with initial randomly choosen spectral function)
  REAL(c_double)      :: variationStrength           = 0.05  ! initial variational magnitude for MonteCarlo search of most probable spectrum in the main loop starting from spectral function of previous MonteCarlo-run
  REAL(c_double)      :: variationStrengthSmooth     = 0.005 ! initial variational magnitude for MonteCarlo search of most probable spectrum in the smoothing procedure after convergence of spectral function is reached

  REAL(c_double)      :: annealingUpdate             = 1.5   ! update factor for annealing parameter exp((-L+alpha*S)/Theta): Theta' = Theta * annealingUpdate, [3], methode by Sandivk
  REAL(c_double)      :: annealingParamWarmup        = 10D0  ! initial annealing temperature for warm-up of a randomly choosen spectral function into a more probable one w.r.t. the data
  REAL(c_double)      :: annealingParam              = 0.001 ! initial annealing temperature for main-loop of finding the most probable spectral function
  REAL(c_double)      :: annealingParamSmooth        = 0.005 ! initial annealing temperature for smoothing procedure after convergence of the spectral function is reached
  !*** Metropolis and annealing parameters for determination of spectral function via Montecarlo


  REAL(c_double)      :: kernelExponentCutoff        = 40D0  ! numerical cut-off when exponent of exponentially damped kernels (exp(-40)<<1) is considered to by asymptotic,
                                                     ! numerically limit to stabalize calculations exp(+40)>>1 (which can happen for negative frequencies)
  REAL(c_double)      :: accuracy                    = 1D-12 ! numerical accuracy used e.g. in the MonteCarlo-Subroutine montecarloSpectralFunction for numerical calculations of logarithm
  ...
  REAL(c_double),DIMENSION(:),ALLOCATABLE           :: maxentCorrFunc
  REAL(c_double),DIMENSION(:),ALLOCATABLE           :: maxentInversCov ! is the inverse covariance matrix of the measured Green's function.
  REAL(c_double),DIMENSION(:,:),ALLOCATABLE         :: kernel          ! kernel, satisfying G(j)=Integral(kernel_ji(w,tau,beta,...)*SpecFunc_i(w) dw), Eq.(3.1)Ref[1]
...
SUBROUTINE w2maxent ( specFunc, specModel, specGrid, NSpec, &
                      ...
                      smoothingMethod, c_logPrefix) bind(C, name='w2maxent')
  USE MMersenneTwister                                      !random number generator
  IMPLICIT NONE

  !***************************************************************************************
  !*** Spectral Function Variables ***
  INTEGER(C_INT32_T), VALUE       :: NSpec           ! number of grid points for spectral function
  REAL(C_DOUBLE), INTENT(INOUT)   :: specFunc(Nspec)        ! spectral function, object of interest
  REAL(C_DOUBLE)                  :: specModel(Nspec)       ! (spectral) model for spectral function (enters in entropy)
  REAL(C_DOUBLE)                  :: specGrid(Nspec)        ! grid for spectral function
  REAL(c_double),     DIMENSION(Nspec)              :: specUnit        ! grid weight for numerical integration
  !*** Spectral Function Variables ***

  !*** Green's Function Variables ***
  INTEGER(C_INT32_T), VALUE             :: NGreen          ! number of grid points for Green's function
  COMPLEX(C_DOUBLE_COMPLEX), INTENT(IN) :: greenFunc(Ngreen)       ! Green's function from DEXPeriment / CT-QMC
  REAL(C_DOUBLE), INTENT(IN)            :: greenInverseCov(Ngreen) ! inverse covariance matrix (1/sigma(i)^2), assumed to be diagonal
  REAL(C_DOUBLE), INTENT(IN)            :: greenGrid(Ngreen)       ! grid for Green's function

  INTEGER                                   :: maxentNCorr
  !*** Green's Function Variables ***

  !*** Kernel Variables ***
  INTEGER(C_INT32_T), VALUE :: kernelMode      ! defines specific shape of kernel (0=imaginary time, )
  REAL(C_DOUBLE), VALUE     :: kernelBeta      ! beta from DEXPeriment (1/[kb*T]) which enters the kernel in imaginary time Green's function.
  !*** Kernel Variables ***

  !*** Maximum Entropy (Bayesian) Parameter ***
  REAL(C_DOUBLE), VALUE  :: alphaPrior          ! initial guess for alpha entering the priori pdf: DEXP(alpha*S), S being the entropy
  REAL(c_double)                                    :: alpha               ! iteratively (hopefully convergent w.r.t. Eq. 4.28 in Ref[1]) alpha
  REAL(c_double),     DIMENSION(Nspec,Nspec)        :: likelihoodCurvature ! curvature tensor of likelihood DEXPonent w.r.t. spectral Function, Eq. 4.9 in Ref[1]
  REAL(c_double)                                    :: bayesPostExponent   ! = -2 alpha S, according to Eq. 4.28 in Ref[1]
  REAL(c_double)                                    :: bayesTrace          ! = Trace ( Lambda(A)_ij*(alpha*1_ij+Lambda(A)_ij)^-1 ), Eq. 4.29, 4.30 in Ref[1]
                                                                !   equals number of good data ...o
  REAL(c_double)                                    :: chiSquared          ! least mean squares value obtained from Monte Carlo algorithm
  !*** Maximum Entropy (Bayesian) Parameter ***

  !*** Monte Carlo & Annelaling Parameter ***
  INTEGER(C_INT64_T), VALUE :: NMonteCarlo     ! number of Monte Carlo steps for maximizing posteriori pdf for the spectral function Pr(A|G)
  INTEGER(C_INT64_T), VALUE :: NAnnealing      ! number of Monte Carlo steps for one annealing (temperature) step
  INTEGER(C_INT32_T), VALUE :: seed            ! initial seed for randomn number generator, 4 byte integer
  !*** Monte Carlo & Annelaling Parameter ***

  !*** Smoothening Parameter ***
  INTEGER(C_INT32_T), VALUE :: NSmooth         ! number of smoothing steps after Bayesian convergence is reached (w.r.t. Eq. 4.28 in Ref[1])
  INTEGER(C_INT32_T), VALUE :: smoothCnt       ! number of spectral grid points, where the average is taken
  INTEGER(C_INT32_T), VALUE :: smoothingMethod ! defines the method, how the local average over the (noisy) spectral function is performed
  ...
  REAL(c_double)                                    :: chiOmegaZero    ! real part of spectral susceptibility (via Kramers Kronig Relation)
  REAL(c_double)                                    :: chiTauZero      ! susceptibility at origin of imaginary time (tau=0 in EQ.5.14 REf[1])
  !*** Susceptibility values - only for Kernel 1 and 2 ***

  !*** File IO Parameter ***
  CHARACTER(kind=C_CHAR), INTENT(IN)         :: c_logPrefix(*)
  CHARACTER(:), ALLOCATABLE                  :: logPrefix ! prefix for DLOG-files for Monte Carlo & Bayesian Inverence
  ...
  i = 0
  do
     if (c_logPrefix(i + 1) == C_NULL_CHAR) exit
     i = i + 1
  end do
  allocate(character(len=i) :: logPrefix)
  do i = i, 1, -1
     logPrefix(i:i) = c_logPrefix(i)
  end do
...
!                      kernelBeta,kernelMode)
  ...
  REAL(c_double),DIMENSION(Nspec),        INTENT(IN)  :: specGrid
  REAL(c_double),DIMENSION(Nspec),        INTENT(IN)  :: specUnit
  !*** Spectral Function Variables ***

  !*** Green's Function Variables ***
  INTEGER,                         INTENT(IN)  :: NGreen
  COMPLEX(c_double_complex),DIMENSION(Ngreen),     INTENT(IN) :: greenFunc
  REAL(c_double),DIMENSION(Ngreen),        INTENT(IN)  :: greenGrid
  REAL(c_double),DIMENSION(Ngreen),        INTENT(IN)  :: greenInverseCov
  !*** Green's Function Variables ***

  !*** Kernel Variables ***
  INTEGER,                           INTENT(OUT) :: maxentNCorr
  REAL(c_double),                            INTENT(IN)  :: kernelBeta
...
SUBROUTINE initLikelihoodCurvature(likelihoodCurvature,specUnit,NSpec,NGreen)
  ...
  REAL(c_double),DIMENSION(Nspec)                    :: specUnit        !  is the spectral grid-weight for numerical integration
  !*** Spectral Function Variables ***
  ...
  !*** Maximum Entropy (Bayesian) Parameter ***
  REAL(c_double),DIMENSION(Nspec,Nspec), INTENT(OUT) :: likelihoodCurvature ! is the curvature tensor d^2 L(i,j) / (dA(i)dA(j)).
  !*** Maximum Entropy (Bayesian) Parameter ***
...
SUBROUTINE bayesianInverence (bayesTrace, bayesPostExponent, converged, alpha, &
  ...
  !*** Spectral Function Variables ***
  INTEGER,                      INTENT(IN)                 :: NSpec        ! is the number of spectral grid points
  REAL(c_double),  DIMENSION(Nspec),    INTENT(IN)  :: specFunc     ! is the current spectral function which should fulfill the convergence-critetion
  REAL(c_double),  DIMENSION(Nspec),    INTENT(IN)  :: specModel    ! is the spectral model which enters in the entropy
  REAL(c_double),  DIMENSION(Nspec),    INTENT(IN)  :: specUnit     ! is the spacing between the grid points
  !*** Spectral Function Variables ***

  !*** Maximum Entropy (Bayesian) Parameter ***
  REAL(c_double)                                    :: alpha        ! enters in the entropic prior DEXP(alpha*entropy) and is updated, if no convergence is reached
  REAL(c_double),                       INTENT(IN)  :: chiSquared   ! chiSquared value for logging
  REAL(c_double),DIMENSION(Nspec,Nspec),INTENT(IN)  :: likelihoodCurvature ! the curvaturetensor of the DLOG-likelihood function w.r.t. specFunc
  REAL(c_double),DIMENSION(Nspec,Nspec)             :: lambdaMatrix ! enters the convergence criterion, defined in Eq. 4.8 in Ref[1]
  REAL(c_double),DIMENSION(Nspec,Nspec)             :: gammaMatrix  ! is the curvature-tensor of the DLOG-posteriori pdf, Q=alpha*S-chi^2/2 and also enters the
  ...
  REAL(c_double)                                    :: entropy      ! the entropy of the spectral function w.r.t. the model, also referenced as S
  REAL(c_double),                       INTENT(OUT) :: bayesTrace   ! Trace( lambdaMatrix_ij * (gammaMatrix_ij)^-1 )
  REAL(c_double),                       INTENT(OUT) :: bayesPostExponent !-2alpha*Entropy
  LOGICAL                                   :: converged    ! true if Bayesian convergence is reached
  REAL(c_double)                                    :: bayesRate    ! local helper: bayesTrace/bayesPostExponent
  !*** Maximum Entropy (Bayesian) Parameter ***
  ...
  REAL(c_double),  DIMENSION(NSpec*64)              :: work         ! helper for LAPACK invoke
  !*** Local Helpers ***
  !***************************************************************************************
...
SUBROUTINE montecarloSpectralFunction (specFunc, specModel, specUnit,           &
  ...
  REAL(c_double),  DIMENSION(Nspec), INTENT(OUT) :: specFunc      ! the spectral function, which is to be optimized
  REAL(c_double),  DIMENSION(Nspec), INTENT(IN)  :: specModel     ! the spectral model entering in the entropic prior
  REAL(c_double),  DIMENSION(Nspec), INTENT(IN)  :: specUnit      ! the spacing between spectral grid points
  REAL(c_double),  DIMENSION(Nspec)              :: specVariation ! allowed variation magnitude of the spectral function (for every grid point)
  ...
  REAL(c_double),  DIMENSION(NGreen)             :: greenCalc       ! the "real" values of the Green's function given the kernel and specFunc
  !*** Green's Function Variables ***

  !*** Kernel Variables ***

  !*** Bayesian Variables ***
  REAL(c_double),                    INTENT(OUT) :: chiSquared    ! DLOG likelihood function, mean of squared deviations of greenFunc from greenCalc
  REAL(c_double),                    INTENT(IN)  :: alpha         ! fixed Bayesian parameter entering the entropic prior
  !*** Bayesian Variables ***

  !*** Montecarlo Variables ***
  INTEGER(C_INT64_T),                   INTENT(IN)  :: NMonteCarlo   ! number of Monte Carlo runs for finding new configuration of specFunc
  INTEGER(C_INT64_T),                   INTENT(IN)  :: NAnnealing    ! number of MonteCarlo runs until a new annealing temperature is defined
  REAL(c_double)                                 :: annealingInit, &   ! initial value for the annealing parameter
                                            annealingBoltzmann ! actual annealing parameter: DEXP(Q/annealingBoltzmann)
  REAL(c_double)                                 :: variationInit, &   ! initial parameter of magnitude of variation of the spectral function
                                            variationFact      ! actual variational parameter used in calculation
  REAL(c_double)                                 :: try,    &     ! number of sweeps in a reagion of specFunc, where the variation is performed
  ...
  REAL(c_double)                                 :: sweepJValue,sweepIValue ! values of specFunc after variation at sweepI and sweepJ
  REAL(c_double),  DIMENSION(NGreen)             :: sweepGreenCalc          ! changed "real" value of the Green's function because of sweep
  REAL(c_double)                                 :: sweepChiSquared         ! changed DLOG-likelihood function because of sweep
  REAL(c_double)                                 :: sweepEntropy            ! change in entropy because of sweep
  REAL(c_double)                                 :: sweepWeight             ! probability ratio before/after sweep
  !*** Montecarlo Variables ***
  ...
  !*** Local Helpers ***
  INTEGER(C_INT64_T)                         :: i,j           ! loop helpers
  !*** Local Helpers ***
  !**************************************************************************************
...
IMPLICIT NONE
  ...
  REAL(c_double),  DIMENSION(Nspec), INTENT(OUT) :: specFunc        ! spectral function, object of interest
  REAL(c_double),  DIMENSION(Nspec), INTENT(IN)  :: specGrid        ! grid for spectral function
  ...
  INTEGER,                   INTENT(IN)  :: kernelMode      !
  REAL(c_double)                                 :: kernelBeta

  REAL(c_double),  DIMENSION(Nspec)              :: specSmooth      ! dummy spectrum for smoothing
  REAL(c_double)                                 :: specWeight      ! dummy spectrum for smoothing
  REAL(c_double)                                 :: smoothRange     ! dummy spectrum for smoothing
...
SUBROUTINE chiMoments (specFunct, grid, gridUnit, NGrid, beta, kernelMode, chiOmegaZero, chiTauZero)
  ...
  REAL(c_double),  DIMENSION(NGrid), INTENT(IN)  :: specFunct     !spectral function for specific kernel
  REAL(c_double),  DIMENSION(NGrid), INTENT(IN)  :: grid          !grid where specFunct is defined, should range from [0,inf]
  REAL(c_double),  DIMENSION(NGrid), INTENT(IN)  :: gridUnit      !grid integration weights
  REAL(c_double),                    INTENT(IN)  :: beta          !Boltzmann factor
  INTEGER,                   INTENT(IN)  :: kernelMode!defines normalization mode
  !*** Function Variables ***


  !*** Return values ***
  REAL(c_double),                    INTENT(OUT) :: chiOmegaZero  ! real part of spectral susceptibility (via Kramers Kronig Relation)
  REAL(c_double),                    INTENT(OUT) :: chiTauZero    ! susceptibility at origin of imaginary time (tau=0 in EQ.5.14 REf[1])
  !*** Return values ***
...
SUBROUTINE getGridUnit (grid,unit,Ngrid)
  ...
  REAL(c_double),  DIMENSION(Ngrid), INTENT(IN)  :: grid
  REAL(c_double),  DIMENSION(Ngrid), INTENT(OUT) :: unit
  INTEGER                                :: i
...
!*****************************************************************************************
SUBROUTINE normalizeToGrid(funct,grid,NGrid,kernelMode) bind(C, name='normalizeToGrid')
  IMPLICIT NONE

  !*** Function Variables ***
  INTEGER(C_INT32_T), VALUE     :: NGrid     !number of grid points
  REAL(C_DOUBLE), INTENT(INOUT) :: funct(NGrid)     !function to be normalized
  REAL(C_DOUBLE), INTENT(IN)    :: grid(NGrid)      !grid where funct is defined
  INTEGER(C_INT32_T), VALUE     :: kernelMode !defines normalization mode
  REAL(c_double),  DIMENSION(NGrid)                :: gridUnit  !spacing between grid points
  !*** Function Variables ***
