...
module type_progress
    ...
    type, public :: progress
        ...
        real(c_double):: updateint = 60.      !< Desired update interval [s]
        real(c_double):: adaptrate = 0.1      !< How fast the system adapts (0, 1)
        ...
        real(c_double):: startrun  = 0        !< Progress run start [s]
        real(c_double):: startintv = 0        !< Current interval start [s]
    ...
    real(c_double), parameter :: RAMP_FACTOR = 1.7, RAMP_THRESHOLD = .1
...
contains
    ...
    subroutine ptick(p)
        ...
        if(p%fancy) then
            eqs = floor(p%ticks*40.0d0/p%target)   ! otherwise risk of overflow
            write(p%unit,"(A,A,1X,I3,'% [',A40,'] ',A6,' (T-',A6,')')", advance="no")&
                achar(13), p%title, floor(p%ticks*100.0d0/p%target), &
                repeat('=',eqs) // DIRCHARS(mod(p%updates,modarg)+1) // repeat(' ',39-eqs),&
                tstr(t), tstr(trem)
        else
            write(p%unit,"(A,I12,' of',I12,' (',F5.1,'%) ',A6,'  T-',A6)") &
                p%title, p%ticks, p%target, p%ticks*100.0d0/p%target, tstr(t), tstr(trem)
        endif
        call cpu_time(p%startintv)
    contains
