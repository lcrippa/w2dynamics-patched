...
module MBLOCK_MATRIX
    ...
    type, public :: BLOCK_MATRIX
        private
        integer :: nblocks = 0, size = 0
        type(EXT_RANGE) :: det
        type(UPDATABLE_MATRIX), allocatable :: block(:)
    ...
    !! concrete update type by setting a pointer of type(BLOCK_UPDATE_BASE)
    !! and then use get_det() and accept_update().
    type, abstract, public :: BLOCK_UPDATE_BASE
        private
        type(BLOCK_MATRIX), pointer :: target
        type(EXT_RANGE) :: det
        logical :: allowed
    ...
    abstract interface
        subroutine base_accept_update(self)
            import :: BLOCK_UPDATE_BASE
            class(BLOCK_UPDATE_BASE), intent(inout) :: self
        end
    end interface
    ...
    !! propose this update.
    type, extends(BLOCK_UPDATE_BASE), public :: BLOCK_REPLACE
        private
        type(MATRIX_REPLACE), allocatable :: block(:)
        integer :: size
        integer, allocatable :: offset(:)
        integer, allocatable :: lookup(:)
    contains
    ...
    type, extends(BLOCK_UPDATE_BASE), public :: BLOCK_GROW
        private
        type(MATRIX_GROW), allocatable :: block(:)
        integer :: size
        integer, allocatable :: offset(:)
        integer, allocatable :: lookup(:)
    contains
    ...
    type, extends(BLOCK_UPDATE_BASE), public :: BLOCK_SHRINK
        private
        type(MATRIX_SHRINK), allocatable :: block(:)
        integer :: size, rank
        integer, allocatable :: offset(:)
        integer, allocatable :: lookup(:)
    contains
    ...
    !! update.
    type, extends(BLOCK_UPDATE_BASE), public :: BLOCK_REPLACE_ROWS
        private
        type(REPLACE_ROWS), allocatable :: block(:)
        integer :: size, rank
    contains
        private
    ...
    !! update.
    type, extends(BLOCK_UPDATE_BASE), public :: BLOCK_REPLACE_COLS
        private
        type(REPLACE_COLS), allocatable :: block(:)
        integer :: size, rank
    contains
        private
    ...
    !! update.
    type, extends(BLOCK_UPDATE_BASE), public :: BLOCK_PERMUTE
        private
        type(MATRIX_PERMUTE), allocatable :: block(:)
        integer :: size
    contains
        private
    ...
    VALUE_TYPE, parameter :: ONE = 1
    VALUE_TYPE, parameter :: ZERO = 0

contains

    pure logical function p_isinit(self)
        type(BLOCK_MATRIX), intent(in) :: self

        p_isinit = allocated(self%block)
    end

    subroutine p_init(self, nblocks)
        type(BLOCK_MATRIX), intent(inout) :: self
    ...
    pure function p_base_det(self) result(r)
        class(BLOCK_UPDATE_BASE), intent(in) :: self
        type(EXT_RANGE) :: r
    ...
    pure logical function p_repl_isinit(self)
        type(BLOCK_REPLACE), intent(in) :: self

        p_repl_isinit = allocated(self%block)
    end

    subroutine p_repl_init(self, nblocks)
        type(BLOCK_REPLACE), intent(inout) :: self
        integer, intent(in) :: nblocks
    ...
    subroutine p_repl_propose(self, t, iblock, jblock, a)
        type(BLOCK_REPLACE), intent(inout) :: self
        type(BLOCK_MATRIX), intent(inout), target :: t
    ...
    subroutine p_repl_accept(self)
        class(BLOCK_REPLACE), intent(inout) :: self
        integer :: b
    ...
    pure function p_grow_isinit(self) result(test)
        type(BLOCK_GROW), intent(in) :: self
        logical :: test
    ...
    subroutine p_grow_init(self, nblocks)
        type(BLOCK_GROW), intent(inout) :: self
        integer, intent(in) :: nblocks
    ...
    subroutine p_grow_accept(self)
        class(BLOCK_GROW), intent(inout) :: self
        integer :: b
    ...
    pure function p_shrink_isinit(self) result(test)
        type(BLOCK_SHRINK), intent(in) :: self
        logical :: test
    ...
    subroutine p_shrink_init(self, nblocks)
        type(BLOCK_SHRINK), intent(inout) :: self
        integer, intent(in) :: nblocks
    ...
    subroutine p_shrink_accept(self)
        class(BLOCK_SHRINK), intent(inout) :: self
        integer :: b
    ...
    pure function p_rows_isinit(self) result(test)
        type(BLOCK_REPLACE_ROWS), intent(in) :: self
        logical :: test
    ...
    subroutine p_rows_init(self, nblocks)
        type(BLOCK_REPLACE_ROWS), intent(inout) :: self
        integer, intent(in) :: nblocks
    ...
    subroutine p_rows_propose(self, t, ifrom, ito, val)
        ...
        VALUE_TYPE :: val_sort(size(val,1), size(val,2))
    ...
    subroutine p_rows_accept(self)
        class(BLOCK_REPLACE_ROWS), intent(inout) :: self
        integer :: b
    ...
    pure function p_cols_isinit(self) result(test)
        type(BLOCK_REPLACE_COLS), intent(in) :: self
        logical :: test
    ...
    subroutine p_cols_init(self, nblocks)
        type(BLOCK_REPLACE_COLS), intent(inout) :: self
        integer, intent(in) :: nblocks
    ...
    subroutine p_cols_accept(self)
        class(BLOCK_REPLACE_COLS), intent(inout) :: self
        integer :: b
    ...
    pure function p_permute_isinit(self) result(test)
        type(BLOCK_PERMUTE), intent(in) :: self
        logical :: test
    ...
    subroutine p_permute_init(self, nblocks)
        type(BLOCK_PERMUTE), intent(inout) :: self
        integer, intent(in) :: nblocks
    ...
    subroutine p_permute_accept(self)
        class(BLOCK_PERMUTE), intent(inout) :: self
        integer :: b
