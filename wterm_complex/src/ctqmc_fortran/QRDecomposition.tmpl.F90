...
module MQR_DECOMPOSITION
    ...
    !! are the factors for the reflectors (output of GEQRF). If pivoting is
...
contains
    ...
    pure function conjg_rot(a) result(r)
        type(PLANE_ROTATION), intent(in) :: a
        type(PLANE_ROTATION) :: r
    ...
    pure function householder_det_one(tau) result(det)
        ...
        if (tau == 0) then
            ...
            det = -tau * tau
#else
            ! For real matrices, this implies that if H actually reflects,
    ...
    subroutine p_qr_hessenberg(k, h, ldh, g)
        ...
        VALUE_TYPE, intent(inout) :: h(:, :)
        type(PLANE_ROTATION), intent(out) :: g(:, :)

        integer :: i, j, l, m, n
        type(PLANE_ROTATION) :: gcurr
        ...
        do j = 1, n - 1
            do l = k, 1, -1
                ...
                if (i >= m) then
                    ...
                    call ROT(n-j, h(i, j+1), ldh, h(i+1, j+1), ldh, &
                             gcurr%c, gcurr%s)
                    g(l, j) = gcurr
                endif
            enddo
    ...
    subroutine p_rotate_q(q, ldq, g)
        ...
        VALUE_TYPE, intent(inout) :: q(:, :)
        type(PLANE_ROTATION), intent(in) :: g(:, :)

        type(PLANE_ROTATION) :: gadj
        integer :: i, j, k, l, m, n
        ...
        do j = 1, n - 1
            do l = k, 1, -1
                ...
                gadj = conjg(g(l, j))
                call ROT(m, q(1, i), 1, q(1, i+1), 1, gadj%c, gadj%s)
            enddo
        enddo
    end subroutine
    ...
    subroutine p_q_diagonalize(q, ldq, giv)
        integer, intent(in) :: ldq
        VALUE_TYPE, intent(inout) :: q(:, :)
        type(PLANE_ROTATION), intent(out) :: giv(:, :)
        ...
        do d = n-1, 1, -1
            do i = 1, m
                ...
                if (j >= n) then
                    ...
                    if (i < m) then
                        call ROT(m - i, q(i+1, j), 1, q(i+1, j+1), 1, &
                                 gadj%c, gadj%s)
                    endif
                    giv(i, d) = conjg(gadj)
                endif
    ...
    subroutine p_rotate_r(r, ldr, giv)
        ...
        VALUE_TYPE, intent(inout) :: r(:, :)
        type(PLANE_ROTATION), intent(in) :: giv(:, :)
        ...
        do d = n-1, 1, -1
            do i = 1, k
                ...
                call ROT(m, r(j, 1), ldr, r(j+1, 1), ldr, gcurr%c, gcurr%s)
            enddo
        enddo
    end subroutine

    subroutine rotate_inplace(f, g, rot)
        VALUE_TYPE, intent(inout) :: f, g
        type(PLANE_ROTATION), intent(out) :: rot
        VALUE_TYPE :: r

        call LARTG(f, g, rot%c, rot%s, r)
        f = r
        g = 0
    end subroutine
