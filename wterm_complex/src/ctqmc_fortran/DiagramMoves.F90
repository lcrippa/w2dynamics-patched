...
contains
    ...
    subroutine perform_move( &
                    ...
                    PercentageOuterMove, Percentage4OperatorMove, wormEta, &
        ...
        real(c_double), intent(in) :: PercentageGlobalMove, PercentageTauShiftMove
        real(c_double), intent(in) :: PercentageOuterMove, Percentage4OperatorMove
        real(c_double), intent(in) :: PercentageWormInsert, PercentageWormReplace
        ...
        if (current_sector == SecZ) then
            ...
            move_type_threshold = move_type_threshold - PercentageTauShiftMove
            ...
            ! MOVE: global (permute)
            move_type_threshold = move_type_threshold - PercentageGlobalMove
            if (move_type_rnd > move_type_threshold) then
        ...
        if (current_sector == SecZ) then
            if (allowed_worm_sector > SecZ) then
                move_type_threshold = move_type_threshold - PercentageWormInsert
                if (move_type_rnd > move_type_threshold) then
            ...
            move_type_threshold = move_type_threshold - PercentageWormReplace
            if (move_type_rnd > move_type_threshold) then
        ...
        if (fourop_rnd >= Percentage4OperatorMove) outer_rnd = random_real(samprng)

        move_type_threshold = move_type_threshold / 2.0d0
        if (move_type_rnd > move_type_threshold) then
            if (fourop_rnd >= Percentage4OperatorMove) then
                ! 1-pair
                move_type = MOVE_TYPE_REMOVE
                call perform_RemHybPair(window, rem_move, diagram, 1, samprng,&
                        outer_rnd < PercentageOuterMove, movestat%remhybpair(1))
                return
            else
                ! 2-pair
            ...
            if (fourop_rnd >= Percentage4OperatorMove) then
                ...
                call perform_AddHybPair(window, add_move, diagram, 1, samprng,&
                        outer_rnd < PercentageOuterMove, movestat%addhybpair(1))
                return
            else
                ! 2-pair
