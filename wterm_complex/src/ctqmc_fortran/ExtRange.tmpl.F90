module MEXT_RANGE
    ...
    interface EXT_RANGE
        ...
        module procedure p_extend_real
#endif
    end interface
...
#ifdef VALUE_IS_COMPLEX
        module procedure p_assign_real
#endif
    end interface
    public assignment ( = )
...
contains
    ...
    pure elemental function p_add(x, y) result(r)
        type(EXT_RANGE), intent(in) :: x, y
        type(EXT_RANGE) :: r
        VALUE_TYPE :: xval, yval
    ...
    pure subroutine p_rescale(x, y, xval, yval, expon)
        type(EXT_RANGE), intent(in) :: x, y
        VALUE_TYPE, intent(out) :: xval, yval
...
#ifdef VALUE_IS_COMPLEX
    pure elemental function p_mul_real(x, y) result(r)
        type(EXT_RANGE), intent(in) :: x
        REAL_TYPE, intent(in) :: y
        VALUE_TYPE :: yc
        type(EXT_RANGE) :: r
    ...
    pure elemental function p_div_real(x, y) result(r)
        type(EXT_RANGE), intent(in) :: x
        REAL_TYPE, intent(in) :: y
        VALUE_TYPE :: yc
        type(EXT_RANGE) :: r
    ...
    pure subroutine p_assign_scalar(self, x)
        type(EXT_RANGE), intent(out) :: self
        VALUE_TYPE, intent(in) :: x
...
#ifdef VALUE_IS_COMPLEX
    pure elemental function p_extend_real(x) result(r)
        type(EXT_RANGE) :: r
    ...
    pure subroutine p_assign_real(self, x)
...
#if VALUE_IS_COMPLEX
        r = cmplx(scale(real(x%val), x%exp), &
                  scale(aimag(x%val), x%exp), &
    ...
    pure subroutine p_split(val, fract, expon)
        VALUE_TYPE, intent(in) :: val
        VALUE_TYPE, intent(out) :: fract
        integer(c_long), intent(out) :: expon
        intrinsic :: fraction, exponent
#if VALUE_IS_COMPLEX
        integer :: re_exp, im_exp
        REAL_TYPE :: r_val_real, r_val_imag
#endif

#if VALUE_IS_COMPLEX
        re_exp = exponent(real(val))
        im_exp = exponent(aimag(val))

        if (re_exp > im_exp) then
            ! The real part is bigger than the imaginary
            expon = re_exp
            r_val_real = fraction(real(val))
            r_val_imag = scale(aimag(val), -re_exp)
        else
            ! The imaginary part is bigger than the real part
            expon = im_exp
            r_val_real = scale(real(val), -im_exp)
            r_val_imag = fraction(aimag(val))
        end if
        fract = cmplx(r_val_real, r_val_imag, kind=c_double_complex)
#else
        fract = fraction(val)
        expon = exponent(val)
#endif
...
#if VALUE_IS_COMPLEX
        write (get_unit(unit), 98) &
                real(x%val) * ffact, exp10, aimag(x%val) * ffact, exp10

    98  format ('(', F19.16, 'E', SP, I0, ',', F19.16, 'E', SP, I0, ')')
#else
    ...
    subroutine tobase10(e2, e10, ffact)
        ...
        REAL_TYPE, intent(out) :: ffact

        REAL_TYPE :: e10r
        REAL_TYPE, parameter :: log10_radix = log10(1.0d0 * radix(1.0d0))
