...
module MHybridizationD
    ...
    !!
    type, abstract, public :: DHybridization
        private
        integer :: norb = -9999
        logical :: fermion
        real(c_double) :: beta
    ...
    interface
        subroutine hybr_matrix(self, iop, jop, out)
            import :: DHybridization, TBaseOper, c_double, c_double_complex
            class(DHybridization), intent(in) :: self
            class(TBaseOper), intent(in) :: iop(:), jop(:)
            real(c_double), intent(out) :: out(:, :)
        end subroutine
    end interface

    type, extends(DHybridization) :: hybr_linear
        private
    ...
    type, extends(DHybridization) :: hybr_log
        private
        type(TLogGrid) :: grid
        real(c_double), allocatable :: mat(:,:,:,:)
    contains
...
contains
    ...
    subroutine init_base_hybridization(self, norb, beta, fermion)
        class(DHybridization), intent(out) :: self
    ...
    subroutine p_linear_init_bandspin_alloc(self, fmat, beta, fermion)
        class(DHybridization), allocatable, intent(out) :: self
        real(c_double), intent(in) :: fmat(:, :, :, :, :)
        real(c_double), intent(in) :: beta
        logical, intent(in) :: fermion
    ...
    subroutine add_propagator(acc, c, w, beta, grid)
        ...
        real(c_double), intent(inout) :: acc(0:grid%spec%k, grid%n_min:grid%n_max, 0:1)
        real(c_double), intent(in) :: c
        real(c_double), intent(in) :: w, beta

        real(c_double) :: p0(0:grid%spec%k)
        real(c_double) :: cpos, cneg
        integer :: n
    ...
    pure function eval_poly3(p, x) result(r)
        real(c_double), intent(in) :: p(0:3)
        real(c_double), intent(in) :: x
        real(c_double) :: xsq
        real(c_double) :: y(0:1), r
    ...
    subroutine p_discretize_propagator(vmat, eps, beta, fermion, fmat)
        ...
        real(c_double), intent(in) :: eps(:), beta
        logical, intent(in) :: fermion
        real(c_double), intent(out) :: fmat(:, :, :, :, 0:)
