...
module MUpdatableMatrixD
    ...
    type, public :: DUpdatableMatrix
        private
        integer :: size = 0
        type(DExtRange) :: det
        real(c_double) :: det_q
        real(c_double), allocatable :: matbuf(:, :)
        real(c_double), allocatable :: qbuf(:, :), rbuf(:, :)
    ...
    !! concrete update type by setting a pointer of type(DMatrixUpdateBase)
    !! and then use get_det() and accept_update().
    type, abstract, public :: DMatrixUpdateBase
        private
        type(DUpdatableMatrix), pointer :: target
        type(DExtRange) :: det
    contains
        private
    ...
    abstract interface
        subroutine base_accept_update(self)
            import :: DMatrixUpdateBase
            class(DMatrixUpdateBase), intent(inout) :: self
        end
    end interface
    ...
    !! propose this update.
    type, extends(DMatrixUpdateBase), public :: DMatrixReplace
        integer :: size = 0
        real(c_double) :: det_q
        real(c_double), allocatable :: matbuf(:, :)
        real(c_double), allocatable :: qrbuf(:, :), taubuf(:), work(:)
    ...
    type, extends(DMatrixUpdateBase), public :: DMatrixGrow
        private
        integer :: newsize, rank
        real(c_double) :: det_q
        real(c_double), allocatable :: rows(:, :), cols(:, :), dot(:, :)
        real(c_double), allocatable :: rbuf(:, :), qbuf(:, :)
        type(DPlaneRotation), allocatable :: giv(:, :)
    ...
    type, extends(DMatrixUpdateBase), public :: DMatrixShrink
        private
        integer :: newsize, rank
        real(c_double) :: det_q
        integer, allocatable :: irem(:), jrem(:), jperm(:)
        real(c_double), allocatable :: qrows(:, :), q(:, :), r(:, :)
        type(DPlaneRotation), allocatable :: giv_q(:, :), giv_r(:, :)
    contains
        private
    ...
    type, extends(DMatrixUpdateBase), public :: DLowRankUpdate
        private
        type(DMatrixReplace) :: naive
    contains
        private
    ...
    !! update.
    type, extends(DMatrixUpdateBase), public :: DReplaceRows
        private
        type(DMatrixReplace) :: naive
    contains
        private
    ...
    !! update.
    type, extends(DMatrixUpdateBase), public :: DReplaceCols
        private
        type(DMatrixReplace) :: naive
    contains
        private
    ...
    !! update.
    type, extends(DMatrixUpdateBase), public :: DMatrixPermute
        private
        integer :: size
        real(c_double) :: det_q
        integer, allocatable :: iperm(:), jperm(:)
        logical, allocatable :: pwork(:)
        real(c_double), allocatable :: iscale(:), jscale(:), buffer(:,:)
    contains
        private
...
contains
    pure logical function p_isinit(self) result(test)
        type(DUpdatableMatrix), intent(in) :: self

        test = allocated(self%matbuf)
    end

    subroutine p_init(self)
        type(DUpdatableMatrix), intent(inout) :: self
    ...
    subroutine p_copyinv(self, buf)
        ...
        real(c_double) :: cbuf(self%size, self%size)
        integer :: info, n
    ...
    pure subroutine copy_adjoint(a, aadj)
        ...
        real(c_double), intent(out) :: aadj(size(a, 2), size(a, 1))
        integer :: i, j
    ...
    subroutine p_verify(self, rtol)
        ...
        if (n /= 0) then
            call DGEMM('C', 'N', self%size, self%size, self%size, ONE, &
    ...
    subroutine p_reconstruct_matrix(self, buf)
        ...
        call DGEMM('N', 'N', self%size, self%size, self%size, ONE, &
                self%qbuf(1, 1), size(self%qbuf, 1), &
                self%rbuf(1, 1), size(self%rbuf, 1), &
    ...
    function p_determinant(a) result(det)
        ...
        real(c_double) :: work(size(a, 1), size(a, 2))
        ...
        work(:, :) = a(:, :)
        call DGETC2(n, work(1, 1), size(work, 1), ipiv(1), jpiv(1), info)
        if (n < 0) &
    ...
    pure function p_base_det(self) result(r)
        class(DMatrixUpdateBase), intent(in) :: self
        type(DExtRange) :: r
    ...
    pure logical function p_repl_isinit(self) result(test)
        type(DMatrixReplace), intent(in) :: self

        test = allocated(self%matbuf)
    end

    subroutine p_repl_init(self)
        type(DMatrixReplace), intent(inout) :: self
    ...
    subroutine p_repl_propose(self, t, a)
        type(DMatrixReplace), intent(inout) :: self
        type(DUpdatableMatrix), intent(inout), target :: t
        real(c_double), intent(in) :: a(:, :)
    ...
    subroutine p_repl_accept(self)
        class(DMatrixReplace), intent(inout) :: self
        integer :: i, j, n, info
        ...
        do j = 1, n-1
            ...
            enddo
        enddo
        call DORGQR(n, n, n, self%target%qbuf(1,1), size(self%target%qbuf, 1), &
    ...
    subroutine p_grow_setsize(self, newsize, rank)
        type(DMatrixGrow), intent(inout) :: self
    ...
    subroutine p_grow_propose_naive(self, t, inew, jnew, rows, cols, dot)
        ...
        real(c_double), intent(in) :: rows(:, :), cols(:, :), dot(:, :)

        integer :: n, k
        integer :: iperm(size(cols,1) + size(cols,2)), jperm(size(iperm))
        real(c_double) :: matbuf(size(iperm), size(iperm))
    ...
    subroutine p_grow_propose(self, t, inew, jnew, rows, cols, dot)
        ...
        real(c_double) :: colsp(size(cols, 1), size(cols, 2))
        integer :: j, n, k
    ...
    subroutine p_grow_accept(self)
        class(DMatrixGrow), intent(inout) :: self
        integer :: n, k
    ...
    subroutine p_shrink_accept(self)
        class(DMatrixShrink), intent(inout) :: self
        integer :: n, k
    ...
    pure logical function p_update_isinit(self) result(test)
        type(DLowRankUpdate), intent(in) :: self

        test = p_repl_isinit(self%naive)
    end

    subroutine p_update_init(self)
        type(DLowRankUpdate), intent(inout) :: self
    ...
    subroutine p_update_propose_naive(self, t, u, vdag, iperm, jperm)
        ...
        real(c_double), intent(in) :: u(:, :), vdag(:, :)

        integer :: n, k
        real(c_double) :: matbuf(t%size, t%size)
        ...
        if (k == 1) then
            ...
            call DGER(n, n, ONE, u(1,1), 1, vdag(1,1), 1, &
                      matbuf(1,1), size(matbuf, 1))
        else
            call DGEMM('U', 'U', n, n, k, &
    ...
    subroutine p_update_accept(self)
        class(DLowRankUpdate), intent(inout) :: self
    ...
    pure logical function p_rows_isinit(self) result(test)
        type(DReplaceRows), intent(in) :: self

        test = p_repl_isinit(self%naive)
    end

    subroutine p_rows_init(self)
        type(DReplaceRows), intent(inout) :: self
    ...
    subroutine p_rows_accept(self)
        class(DReplaceRows), intent(inout) :: self
    ...
    pure logical function p_cols_isinit(self) result(test)
        type(DReplaceCols), intent(in) :: self

        test = p_repl_isinit(self%naive)
    end

    subroutine p_cols_init(self)
        type(DReplaceCols), intent(inout) :: self
    ...
    subroutine p_cols_accept(self)
        class(DReplaceCols), intent(inout) :: self
    ...
    pure logical function p_permute_isinit(self) result(test)
        type(DMatrixPermute), intent(in) :: self

        test = allocated(self%iperm)
    end

    subroutine p_permute_init(self)
        type(DMatrixPermute), intent(inout) :: self
    ...
    subroutine p_permute_propose(self, t, iperm, jperm, iscale, jscale)
        ...
        type(DUpdatableMatrix), intent(inout), target :: t
        integer, intent(in) :: iperm(:), jperm(:)
        real(c_double), intent(in) :: iscale(:), jscale(:)

        integer :: n
        real(c_double) :: prod_iscale, prod_jscale
    ...
    subroutine p_permute_accept(self)
        class(DMatrixPermute), intent(inout) :: self
    ...
    subroutine p_scale_rows(a, si)
        ...
        real(c_double), intent(in) :: si(size(a,1))
        integer :: i, j
    ...
    subroutine p_scale_cols(a, sj)
        real(c_double), intent(inout) :: a(:, :)
        real(c_double), intent(in) :: sj(size(a,2))
        integer :: i, j
    ...
    subroutine scalematrix(a, si, sj)
        real(c_double), intent(inout) :: a(:, :)
        real(c_double), intent(in) :: si(size(a,1)), sj(size(a,2))
        integer :: i, j
    ...
    subroutine p_permutematrix(ain, iperm, jperm, aout)
        real(c_double), intent(in) :: ain(:, :)
        real(c_double), intent(out) :: aout(:, :)
        integer, intent(in) :: iperm(:), jperm(:)
    ...
    subroutine reserve_plane_rot(buf, rowcap, colcap)
        type(DPlaneRotation), allocatable, intent(inout) :: buf(:,:)
        integer, intent(in) :: rowcap, colcap
        type(DPlaneRotation), allocatable :: tmp(:,:)
