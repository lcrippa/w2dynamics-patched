...
module MIMPURITY
    ...
    type, public :: IMPURITY
        private
        REAL_TYPE :: beta = -1
        type(OPER_EIGEN) :: hloc
        type(PSIS) :: psis

        type(EXT_RANGE) :: weight
        integer :: size = 0
    ...
    type, abstract, public :: IMPURITY_UPDATE_BASE
        private
        type(IMPURITY), pointer :: target
        type(EXT_RANGE) :: weight
    ...
    abstract interface
        subroutine base_accept_update(self)
            import :: IMPURITY_UPDATE_BASE
            class(IMPURITY_UPDATE_BASE), intent(inout) :: self
        end
    end interface

    type, extends(IMPURITY_UPDATE_BASE), public :: IMPURITY_REPLACE
        private
        integer :: size
        type(impurity_pos), allocatable :: pos(:)
        type(KET) :: ket_work
    ...
    type, abstract, extends(IMPURITY_UPDATE_BASE) :: naive_update
        private
        type(IMPURITY_REPLACE) :: naive
    contains
        private
        procedure :: accept_update => p_naive_accept
    end type

    type, extends(IMPURITY_UPDATE_BASE), public :: IMPURITY_GROW
        private
    ...
    type, extends(IMPURITY_UPDATE_BASE), public :: IMPURITY_SHRINK
        private
        integer :: new_size, rank
    ...
    type, extends(naive_update), public :: IMPURITY_SHIFT
        private
        integer :: new_first
        integer, allocatable :: perm(:)
    ...
    type, extends(naive_update), public :: IMPURITY_UPDATE
        private
        integer :: rank
    ...
    VALUE_TYPE, parameter :: ZERO = 0, ONE = 1
...
contains
    ...
    subroutine p_init_impurity(self, beta, hloc, psis, outer_sst)
        type(IMPURITY), intent(out) :: self
        REAL_TYPE, intent(in) :: beta
        type(OPER_EIGEN), intent(in) :: hloc
        type(PSIS), intent(in) :: psis
    ...
    subroutine set_to_empty(self, outer_sst)
        type(IMPURITY), intent(inout) :: self
        integer, intent(in) :: outer_sst
    ...
    subroutine p_set_impurity(self, ops, outer_sst)
        type(IMPURITY), intent(inout), target :: self
        type(TLocalOper), intent(in) :: ops(:)
        integer, intent(in) :: outer_sst
        type(IMPURITY_REPLACE) :: move
    ...
    function p_allowed_outer_states(self) result(r)
        type(IMPURITY), intent(in), target :: self
        integer, pointer :: r(:)
    ...
    function get_psi_for_localoper(psis, op) result(psi)
        type(PSIS), intent(in), target :: psis
        type(TLocalOper), intent(in) :: op
        type(OPERATOR), pointer :: psi
    ...
    subroutine p_compute_density_matrix(self, sgn, out_op)
        type(IMPURITY), intent(in) :: self
        VALUE_TYPE, intent(in) :: sgn
        type(OPERATOR), intent(inout) :: out_op

        integer :: i
        real(c_double) :: delta_tau
        type(EXT_RANGE) :: alpha
    ...
    subroutine init_update_base(self, t)
        class(IMPURITY_UPDATE_BASE), intent(out) :: self
        type(IMPURITY), intent(inout), target :: t
    ...
    pure logical function p_is_update_allowed(self) result(test)
        class(IMPURITY_UPDATE_BASE), intent(in) :: self

        test = self%outer_sst /= -1
    end
    ...
    subroutine p_init_replace_impurity(self, t)
        type(IMPURITY_REPLACE), intent(out) :: self
        type(IMPURITY), intent(inout), target :: t
    ...
    subroutine p_propose_replace_impurity(self, op, outer_sst)
        type(IMPURITY_REPLACE), intent(inout) :: self
        type(TLocalOper), intent(in) :: op(:)
    ...
    function p_repl_get_operators(self) result(ops)
        type(IMPURITY_REPLACE), intent(in) :: self
        type(TLocalOper), allocatable :: ops(:)
        integer :: i
    ...
    function p_repl_allowed_outer_states(self) result(r)
        class(IMPURITY_REPLACE), intent(inout), target :: self
        integer, pointer :: r(:)
    ...
    logical function p_repl_is_allowed_outer(self, sst) result(is_allowed)
        class(IMPURITY_REPLACE), intent(inout) :: self
        integer, intent(in) :: sst
    ...
    subroutine p_repl_choose(self, outer_sst)
        ...
        if (outer_sst /= outer_sst_final .or. outer_sst == -1) then
            self%weight = ZERO
            self%outer_sst = -1
            return
        endif
        ...
        if (self%size == 0) then
            self%weight = partition_function(&
                            self%target%hloc, self%target%beta, outer_sst)
            return
        endif

        call first_after(self%target, self%pos(1), outer_sst)
        call fill_after(self%target, self%pos(:self%size-1), self%ket_work)

        call last_before(self%target, self%pos(self%size), outer_sst)
    ...
    subroutine fill_after(imp, pos, ket_work)
        type(IMPURITY), intent(in) :: imp
        type(impurity_pos), intent(inout) :: pos(:)
        type(KET), intent(inout) :: ket_work
    ...
    subroutine fill_before(imp, pos, bra_work)
        type(IMPURITY), intent(in) :: imp
        type(impurity_pos), intent(inout) :: pos(:)
        type(BRA), intent(inout) :: bra_work
    ...
    subroutine copy_move_from_naive(self)
        class(naive_update), intent(inout) :: self

        self%target => self%naive%target
        self%weight = self%naive%weight
    end
    ...
    subroutine p_init_grow_impurity(self, t)
        type(IMPURITY_GROW), intent(out) :: self
        type(IMPURITY), intent(inout), target :: t
    ...
    subroutine p_propose_grow_impurity(self, ops, outer_sst)
        type(IMPURITY_GROW), intent(inout) :: self
        type(TLocalOper), intent(in) :: ops(:)
        integer, intent(in), optional :: outer_sst

        REAL_TYPE :: old_tau(self%target%size), instau(size(ops))
    ...
    subroutine grow_inner_proposal(self)
        ...
        start_front = self%inspos_ord(1) == 1
        if (start_front) then
            self%istart = 1
            istart_old = -1
        ...
        end_back = self%inspos_ord(self%rank) == self%new_size
        if (end_back) then
    ...
    subroutine grow_outer_proposal(self)
        ...
        call fill_after(self%target, self%pos(1:self%new_size-1), self%work_ket)
        call last_before(self%target, self%pos(self%new_size), self%outer_sst)

        self%weight = sandwich( &
                self%target, self%pos(self%new_size-1:self%new_size), &
                self%work_ket)
    ...
    function grow_outer_state_for_outer_proposal(move) result(outerbl)
        type(IMPURITY_GROW), intent(in) :: move
    ...
    function grow_get_operators(self) result(ops)
        type(IMPURITY_GROW), intent(in) :: self
        type(TLocalOper), allocatable :: ops(:)
    ...
    subroutine grow_accept(self)
        class(IMPURITY_GROW), intent(inout) :: self
        integer :: inew, iold
    ...
    subroutine p_init_shrink_impurity(self, t)
        type(IMPURITY_SHRINK), intent(out) :: self
        type(IMPURITY), intent(inout), target :: t
    ...
    subroutine p_propose_shrink_impurity(self, idx, outer_sst)
        type(IMPURITY_SHRINK), intent(inout) :: self
    ...
    subroutine shrink_inner_proposal(self)
        ...
        start_front = self%irem(1) == 1
        if (start_front) then
            self%istart = 1
            all_front = self%irem(self%rank) == self%rank
            if (all_front) then
        ...
        end_back = self%irem(self%rank) == self%target%size
        if (end_back) then
            self%iend = self%new_size
            iend_old = -1
            all_back = self%irem(1) == self%new_size + 1
            if (all_back) then
    ...
    subroutine shrink_outer_proposal(self)
        ...
        if (self%new_size == 0) then
            ...
            call first_after(self%target, self%pos(1), self%outer_sst)
            call fill_after(&
                    self%target, self%pos(1:self%new_size-1), self%work_ket)
            call last_before(&
                    self%target, self%pos(self%new_size), self%outer_sst)
    ...
    function shrink_outer_state_for_outer_proposal(move) result(outerbl)
        type(IMPURITY_SHRINK), intent(in) :: move
        integer :: outerbl, fixedsst, chnewpos
        real(c_double) :: chtau
        type(TLocalOper), allocatable :: old_ops(:), new_ops(:)
    ...
    function shrink_get_operators(self) result(ops)
        type(IMPURITY_SHRINK), intent(in) :: self
        type(TLocalOper), allocatable :: ops(:)
    ...
    subroutine shrink_accept(self)
        class(IMPURITY_SHRINK), intent(inout) :: self
    ...
    subroutine p_init_shift_impurity(self, t)
        type(IMPURITY_SHIFT), intent(out) :: self
        type(IMPURITY), intent(inout), target :: t
    ...
    subroutine p_propose_shift_impurity(self, delta_tau)
        type(IMPURITY_SHIFT), intent(inout) :: self
        ...
        call propose_replace_impurity(self%naive, ops_upd, outer_sst)
        call copy_move_from_naive(self)
    end

    integer function p_get_num_wrapped_ops(self) result(n)
        type(IMPURITY_SHIFT), intent(in) :: self
    ...
    subroutine p_init_update_impurity(self, t)
        type(IMPURITY_UPDATE), intent(out) :: self
        type(IMPURITY), intent(inout), target :: t
    ...
    subroutine p_propose_update_impurity(self, idx, ops, outer_sst)
        type(IMPURITY_UPDATE), intent(inout) :: self
        integer, intent(in) :: idx(:)
        type(TLocalOper), intent(in) :: ops(:)
        integer, intent(in), optional :: outer_sst
        ...
        REAL_TYPE :: taus(self%target%size)
    ...
    subroutine p_update_get_newidx_addorder(move, out)
        type(IMPURITY_UPDATE), intent(in) :: move
        integer, allocatable, intent(out) :: out(:)
