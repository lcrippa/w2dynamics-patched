...
module MBATH
    ...
    type, extends(BATH_MOVE_BASE), public :: BATH_REPLACE_MOVE
        ...
        type(TBathOperator), allocatable :: ops(:) !> properties of new creators
 ...
 contains
    ...
    subroutine init_bath_lookups(self)
        ...
        sortidx = get_group_perm(whichblock)
    ...
    subroutine p_propose_bath_move(move, crea, annh)
        ...
        VALUE_TYPE :: mat(size(annh), size(crea))
        integer :: crea_ins_perm(size(crea)), annh_ins_perm(size(annh))
        integer :: crea_blocks(size(crea)), annh_blocks(size(annh)), i

        if (size(crea) /= size(annh)) &
            error stop 'Size of creators and annihilators must match'

        move%newsize = size(crea)
        call setsize(move, move%newsize)

        crea_ins_perm = get_sort_perm(get_sortval(move%parent, crea))
        annh_ins_perm = get_sort_perm(get_sortval(move%parent, annh))

        move%crea(:move%newsize) = crea(crea_ins_perm)
        move%annh(:move%newsize) = annh(annh_ins_perm)

        do i = 1, move%newsize
            crea_blocks(i) = move%parent%whichblock(move%crea(i)%orb, move%crea(i)%sp)
            annh_blocks(i) = move%parent%whichblock(move%annh(i)%orb, move%annh(i)%sp)
        end do

        call copy_hybr_matrix(move%parent%hybr, &
                        move%annh(:move%newsize), &
                        move%crea(:move%newsize), &
                        mat)
        call propose_replace_matrix(move%inner, move%parent%mat, &
                                    annh_blocks, crea_blocks, mat)
    end subroutine
    ...
    subroutine p_propose_bath_add_move(move, crea, annh)
        ...
        move%newsize = move%currsize + move%nadd
        call setsize(move, move%newsize, move%nadd)
        ...
        do i = 1, move%nadd
            crea_blocks(i) = move%parent%whichblock(crea_sorted(i)%orb, crea_sorted(i)%sp)
            annh_blocks(i) = move%parent%whichblock(annh_sorted(i)%orb, annh_sorted(i)%sp)
        end do
        ...
        call sorted_insert( &
                get_sortval(move%parent, move%crea(:move%currsize)), &
                get_sortval(move%parent, crea_sorted), &
        ...
        call sorted_insert( &
                get_sortval(move%parent, move%annh(:move%currsize)), &
                get_sortval(move%parent, annh_sorted), &
    ...
    subroutine p_propose_bath_rem_move(move, remidx)
        ...
        call shrink_perm(move%currsize, move%crea_pos(:move%nrem), move%crea_perm(:move%currsize))
        call shrink_perm(move%currsize, move%annh_pos(:move%nrem), move%annh_perm(:move%currsize))

        move%crea(:move%currsize) = move%parent%crea(move%crea_perm(:move%currsize))
        move%annh(:move%currsize) = move%parent%annh(move%annh_perm(:move%currsize))

        call propose_shrink_matrix( &
                    move%inner, move%parent%mat, &
                    move%annh_pos(:move%nrem), move%crea_pos(:move%nrem))
    end subroutine p_propose_bath_rem_move
    ...
    subroutine p_init_bathreplacemove(self, parent)
        ...
        self%parent => parent
        allocate(self%ops(2))
        allocate(self%move_perm(2))
        allocate(self%curr_pos(1))
        allocate(self%new_pos(1))
        allocate(self%repl_rows(1,2))
        allocate(self%repl_cols(2,1))
    end subroutine
    ...
    subroutine p_propose_bathreplacemove(move, origidx, origtype, newop)
        ...
        move%currsize = move%parent%noper/2
        move%nmove = 1
        call setsize(move, move%currsize, move%nmove)
    ...
    end subroutine p_propose_bathreplacemove
    ...
    subroutine p_setsize_shift(self, newsize)
        ...
        currsize = size(self%crea)

        if (newsize > currsize) then
            ! TODO allow higher ranks
            deallocate(self%crea)
            deallocate(self%annh)
            deallocate(self%crea_perm)
            deallocate(self%annh_perm)

            allocate(self%crea(newsize))
            allocate(self%annh(newsize))
            allocate(self%crea_perm(newsize))
            allocate(self%annh_perm(newsize))
        endif
    end subroutine
    ...
    subroutine p_propose_bath_shift_move(move, delta_tau)
        ...
        move%currsize = move%parent%noper/2
        call setsize(move, move%currsize)
        ...
        do i = 1, move%currsize
            ...
            move%crea(i)%tau = move%crea(i)%tau - delta_tau
        ...
        do i = 1, move%currsize
            ...
            move%annh(i)%tau = move%annh(i)%tau - delta_tau
        ...
        call propose_permute_matrix( &
                ...
                iscale, jscale)
    end subroutine p_propose_bath_shift_move
